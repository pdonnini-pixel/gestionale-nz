...
> (165 agenti totali). Dei **149 finding grezzi**: **112 confermati** (dopo accorpamento di 28 duplicati
> segnalati da più aree), **9 scartati** come falsi positivi / già mitigati / dead code.
> Nota operativa: 49 verifiche avversariali sono state completate direttamente dalla sessione principale
> (leggendo il codice citato) perché i subagent hanno raggiunto il limite di spesa mensile a fine run.
...
| 🟡 Media | 58 |
| 🟢 Bassa | 21 |
| **Totale confermati** | **112** |

### I 10 interventi con il miglior rapporto impatto/sforzo

1. **StoreManager**: dropdown cambio-outlet solo hover (critica) + griglie senza breakpoint — è la pagina fatta per Sabrina/Veronica da telefono.
2. **ScadenzarioSmart**: top bar non responsive (overflow orizzontale dell'intera pagina /scadenzario, in bottom nav mobile).
3. **Bottom nav vs elementi flottanti**: pulsante ? dell'aiuto e barra pagamenti bulk si sovrappongono alla bottom nav; classe `safe-area-pb` usata ma mai definita.
4. **Layout root `h-screen`** → `h-dvh` (contenuto in basso coperto dalla barra del browser mobile).
5. **Toast**: backdrop invisibile che ruba il primo tap per 10s + larghezza fissa 384px.
6. **Tabelle senza `overflow-x-auto`** (FinanziamentiTab, Outlet staff, Impostazioni, ArchivioDocumenti, ReportSincronizzazioni, CashflowProspettico): applicare il pattern già usato in Fornitori/RevisionePagamenti.
7. **Griglie KPI a colonne fisse** (`grid-cols-4/5` senza varianti sm:): pattern ripetuto in ~6 pagine.
8. **Tooltip / hover-only**: dare un'attivazione touch (tap/focus) a Tooltip.tsx e alle azioni visibili solo con `group-hover`.
9. **Input con `text-sm`** (14px) → 16px su mobile per eliminare lo zoom automatico iOS su tutti i form.
10. **NotificationBell**: pannello 400px fissi → `max-w-[calc(100vw-1rem)]`.

---

## 🔴 CRITICA — 1 finding

### CRITICA-1 · `src/pages/StoreManager.tsx:189` — Dropdown selezione outlet apribile solo con hover
...
### ALTA-8 · `src/components/Layout.tsx:307` — Root h-screen (100vh) con scroll interno: viewport mobile sbagliato e barra browser mai ritratta

*Area: Gesti e scroll, Orientamento e viewport · Verifica: manuale*
...
### ALTA-9 · `src/components/NotificationBell.tsx:159` — Pannello notifiche largo 400px fisso: tagliato sui viewport 360-430px
...
### ALTA-13 · `src/pages/CashflowProspettico.tsx:1626` — Modal previsione uscita senza max-h né scroll interno
...
### ALTA-14 · `src/pages/Dashboard.tsx:173` — Waterfall di 10+ query Supabase sequenziali al caricamento della Dashboard
...
### ALTA-16 · `src/pages/Fatturazione.tsx:504` — Tabelle fatture 8 colonne con min-width esplicite e Azioni in coda
...
### ALTA-17 · `src/pages/Fornitori.tsx:31` — PdfViewer (pdfjs-dist) importato staticamente: ~350KB gzip pagati all'apertura pagina
...
### ALTA-18 · `src/pages/Fornitori.tsx:849` — Tabella fornitori min-w-[920px]: 2,5 schermate di scroll senza colonna sticky
...
### ALTA-19 · `src/pages/Login.tsx:149` — Tutti gli input hanno font 14px (text-sm): zoom automatico iOS a ogni focus
...
### ALTA-20 · `src/pages/Scadenzario.tsx:1154` — Tabella scadenze 8 colonne con azioni Salda/Rateizza fuori viewport, nessuna vista mobile
...
### ALTA-21 · `src/pages/ScadenzarioSmart.tsx:201` — Nessun blocco dello scroll del body quando i modali sono aperti (scroll bleed)
...
### ALTA-22 · `src/pages/ScadenzarioSmart.tsx:2543` — Top bar Scadenze non responsive: overflow orizzontale dell'intera pagina
...
### ALTA-23 · `src/pages/ScadenzarioSmart.tsx:2670` — KPI 'Riepilogo banche' in grid-cols-5 fissa: colonne da ~55px su mobile
...
### ALTA-24 · `src/pages/ScadenzarioSmart.tsx:2672` — text-slate-400 (contrasto ~3:1) usato massicciamente per testo informativo
...
### ALTA-25 · `src/pages/ScadenzarioSmart.tsx:3627` — Dropdown stato/categoria clippati dal contenitore overflow-x-auto della tabella
...
### ALTA-26 · `src/pages/ScadenzarioSmart.tsx:4035` — Barra azioni bulk pagamenti fixed bottom-6 finisce sotto la bottom nav mobile e copre le ultime righe
...
### ALTA-27 · `src/pages/ScadenzeFiscali.tsx:107` — Input con font 14px (text-sm): iOS Safari fa auto-zoom al focus su tutti i form
...
### ALTA-28 · `src/pages/ScadenzeFiscali.tsx:340` — Azioni di scrittura che falliscono in totale silenzio (nessun toast, nessun alert)
...
### ALTA-29 · `src/pages/StoreManager.tsx:209` — StoreManager: grid-cols-12 e KPI grid-cols-4 senza alcun breakpoint
...
### ALTA-30 · `src/pages/TesoreriaManuale.tsx:450` — Modali con max-h-[90vh]/[92vh] in unità vh: footer e bottoni tagliati dietro la barra browser (pattern in ~18 file)
...
### ALTA-31 · `src/pages/TesoreriaManuale.tsx:875` — Griglie form fisse (grid-cols-2/3) dentro le modali: campi ~75px su smartphone
...
### ALTA-32 · `src/pages/TesoreriaManuale.tsx:885` — Campi importo type=number + parseFloat: la virgola decimale italiana produce 0 silenzioso
...
### MEDIA-1 · `src/components/AccountDetail.tsx:418` — Scroll-lock su document.body inefficace: il vero scroller e' <main>

*Area: Gesti e scroll · Verifica: manuale*

**Problema (mobile):** AccountDetail e' l'unico componente che prova a bloccare lo scroll di sfondo quando il modale e' aperto, ma imposta `document.body.style.overflow = 'hidden'` (righe 418-422). Il body pero' non scrolla mai: il Layout e' `h-screen overflow-hidden` e lo scroll vive su `<main className="flex-1 overflow-y-auto">` (Layout.tsx:346). Il lock quindi non ha alcun effetto: su touch la pagina dietro il modale continua a scorrere (chaining, vedi finding sul Modal di Scadenzario) e tutti gli ALTRI modali dell'app non hanno nemmeno questo tentativo di lock.

**Fix proposto:** Sostituire il lock sul body con `overscroll-contain` sul contenuto scrollabile del modale; se si vuole un lock vero, esporre da Layout un ref/id del `<main>` (es. `document.getElementById('app-main')`) e impostare `overflow: hidden` su quello finche' un overlay e' aperto (utile a tutti i modali, non solo AccountDetail).

### MEDIA-2 · `src/components/AICategorization.tsx:637` — text-slate-300 (contrasto ~1.9:1) su testo con significato di stato
...
### MEDIA-3 · `src/components/ExportBilancioDialog.tsx:23` — jsPDF + jspdf-autotable + xlsx caricati con la pagina Budget anche senza export
...
### MEDIA-4 · `src/components/FinancialTooltip.tsx:63` — Popover glossario largo 256px centrato sull'icona: esce dallo schermo ai bordi
...
### MEDIA-5 · `src/components/GlobalSearch.tsx:65` — Ricerca globale: ESC non chiude in modalita' controllata e mancano le semantiche dialog

*Area: Accessibilità mobile · Verifica: manuale*

**Problema (mobile):** L'handler che chiude con Escape (riga 71) e' registrato solo se `openProp === undefined` (early return riga 65). Layout.tsx:363 passa sempre `open={searchOpen}`, quindi in produzione Escape NON chiude mai l'overlay, mentre l'UI mostra il tasto `<kbd>ESC</kbd>` (riga 175) come suggerimento. L'overlay inoltre non ha role="dialog"/aria-modal e non restituisce il focus al bottone di ricerca alla chiusura. [Verifica: Confermato: in Layout GlobalSearch e' usato in modalita' controllata (open={searchOpen}) e il listener ESC e' attivo solo in modalita' non controllata.]
...
### MEDIA-6 · `src/components/HelpPanel.tsx:211` — Elenchi puntati delle guide collassati in un unico blocco (manca whitespace-pre-line)

*Area: Guide e testi utente · Verifica: manuale*

**Problema (mobile):** Il body delle sezioni guida è renderizzato in `<p className="text-sm text-slate-600 leading-relaxed">{s.body}</p>` senza `whitespace-pre-line`. Ma in src/data/pageGuides.ts almeno 3 body usano `\n\n•` per elenchi puntati: riga 1375 (/import-hub, 'Le fonti di importazione disponibili'), riga 2102 e riga 2110 (/report-sincronizzazioni). In HTML i \n collassano in spazi: l'utente vede un unico paragrafo con i pallini '•' incollati nel testo, illeggibile soprattutto nel pannello largo ~312px su un telefono da 360px. Nota: la chat AI ha già `whitespace-pre-wrap` (riga 147), la tab Guida no. [Verifica: Declassato: i body con elenchi a capo (\n) sono ~3 (es. guida Report Sincronizzazioni, pageGuides.ts:2102) e vengono collassati in un unico blocco senza whitespace-pre-line.]
...
### MEDIA-7 · `src/components/InvoiceViewer.tsx:213` — Tabelle del viewer fattura (6 colonne) senza scroll orizzontale nel modale
...
### MEDIA-8 · `src/components/Layout.tsx:86` — Breadcrumb nascosto su mobile: nessuna indicazione di posizione nell'header
...
### MEDIA-9 · `src/components/Layout.tsx:100` — Bottom nav ignora ruoli e terminologia tenant
...
### MEDIA-10 · `src/components/Layout.tsx:101` — Bottom nav mobile con label 'Outlet' hardcoded, ignora la terminologia del tenant

*Area: Coerenza multi-tenant · Verifica: manuale*

**Problema (mobile):** BottomNav (visibile SOLO su mobile, md:hidden) ha `{ to: '/outlet', icon: Store, label: 'Outlet' }` hardcoded, mentre sidebar e breadcrumb costruiscono le label con useCompanyLabels (Sidebar.tsx buildSections, righe 80-81: posSingular/posPlural). Su Made/Zago, onboardati con label default 'Punto vendita' (Onboarding.tsx:276), la tab principale della navigazione smartphone mostra un termine ('Outlet') che non esiste in nessun'altra parte della loro UI (sidebar dice 'Punto vendita'/'Punti vendita'): incoerenza terminologica per-tenant visibile SOLO su mobile, cioe' esattamente dove lavorano le store manager. Stesso pattern in src/components/GlobalSearch.tsx:26 (chip categoria `label: 'Outlet'` hardcoded, mentre il placeholder alla riga 172 usa correttamente labels.pointOfSalePluralLower).

**Fix proposto:** In BottomNav chiamare useCompanyLabels() e usare `label: labels.pointOfSale` (con truncate/text-ellipsis sullo span text-[10px] per label lunghe come 'Punto vendita'); in GlobalSearch.tsx:26 spostare la costruzione di SEARCH_CATEGORIES dentro il componente usando labels.pointOfSalePlural.

### MEDIA-11 · `src/components/NotificationBell.tsx:208` — Target touch minuscoli annidati in righe cliccabili (X da ~17px)
...
### MEDIA-13 · `src/components/OutletWizard.tsx:656` — Bottoni di chiusura X con touch target 26-33px (< 44px raccomandati)
...
### MEDIA-14 · `src/components/PdfViewer.tsx:218` — PDF zoomato: lato sinistro irraggiungibile (overflow-auto + justify-center) e nessun pinch-to-zoom
...
### MEDIA-15 · `src/components/Sidebar.tsx:511` — Sidebar desktop h-screen attiva in landscape su smartphone: bottone 'Esci' fuori schermo

*Area: Orientamento e viewport · Verifica: avversariale*

**Problema (mobile):** `hidden md:flex h-screen ...`: il breakpoint md (768px) scatta anche su uno smartphone in LANDSCAPE (es. iPhone 14/15 Pro Max = 932x430, Pixel/Galaxy ≥ 780px), dove l'altezza visibile è ~300-330px ma h-screen vale 100vh (~390-430px). Il footer della sidebar con nome utente e bottone 'Esci' (righe 483-503, shrink-0 in fondo al flex-col) finisce quindi sotto il bordo visibile ed è irraggiungibile, perché lo scroll è solo sulla `<nav>` interna (riga 446). Nessuna media query orientation nel progetto per gestire il caso.

**Fix proposto:** Sostituire `h-screen` con `h-dvh` (coerente col fix di Layout.tsx:307). Il drawer mobile (righe 519-521, absolute inset-based dentro fixed inset-0) è invece corretto e non va toccato.

### MEDIA-16 · `src/components/Sidebar.tsx:519` — Drawer mobile della sidebar senza semantica dialog ne' gestione focus
...
### MEDIA-18 · `src/components/ui/KpiCard.tsx:91` — Testo troncato con solo attributo title nativo: informazione persa su touch
...
### MEDIA-20 · `src/data/pageGuides.ts:68` — Guida Dashboard: 'Passando il mouse sul grafico' — istruzione impossibile su touch
...
### MEDIA-22 · `src/pages/AcubeFatturaForm.tsx:148` — Form a colonne fisse (grid-cols-3/2) senza breakpoint: input da ~90px
...
### MEDIA-23 · `src/pages/AnalyticsPOS.tsx:467` — Etichette esterne della torta lunghe (nome + valore) tagliate ai bordi su 360px
...
### MEDIA-24 · `src/pages/AnalyticsPOS.tsx:526` — BarChart raggruppato 12 mesi x N outlet: barre da ~2px illeggibili su smartphone
...
### MEDIA-25 · `src/pages/ArchivioDocumenti.tsx:1353` — Azioni 'Estendi'/'Archivia' conservazione visibili solo su hover della riga
...
### MEDIA-26 · `src/pages/BudgetControl.tsx:1932` — Toast di pagina fixed bottom-6 right-6 z-50: su mobile coprono la bottom nav e spariscono dietro la tastiera
...
### MEDIA-27 · `src/pages/BudgetControl.tsx:2337` — Trigger popover 'Copia mese' di ~16px e popover con controlli minuscoli
...
### MEDIA-28 · `src/pages/BudgetControl.tsx:3234` — Matrice corrispettivi: una colonna di input numerici per ogni outlet, inserimento touch difficile
...
### MEDIA-29 · `src/pages/CashFlow.tsx:436` — Titoli h1 text-4xl/3xl non responsivi che sprecano il viewport mobile
...
### MEDIA-30 · `src/pages/CashFlow.tsx:572` — Soglia critica 50.000 EUR disegnata con <line> SVG grezzo: non viene mai visualizzata
...
### MEDIA-31 · `src/pages/CashflowProspettico.tsx:1797` — interval={0} forza tutte le etichette dell'asse X: sovrapposizione su 360px
...
### MEDIA-32 · `src/pages/ConfrontoOutlet.tsx:1294` — Asse X con 7 nomi outlet: tick auto-nascosti e barre non identificabili senza legenda
...
### MEDIA-33 · `src/pages/ContoEconomico.tsx:3101` — Confronto YoY: importi anno precedente a 10px e delta % a 9px
...
### MEDIA-34 · `src/pages/Dipendenti.tsx:42` — Import statico di lib/pdfText vanifica il lazy loading di PdfViewer in Dipendenti
...
### MEDIA-35 · `src/pages/Fatturazione.tsx:519` — Righe tabella e card cliccabili senza role/tabIndex/tastiera (pattern diffuso)
...
### MEDIA-36 · `src/pages/Impostazioni.tsx:265` — Form soci: label solo come placeholder e riga a larghezze fisse che sborda su 360px
...
### MEDIA-37 · `src/pages/Impostazioni.tsx:629` — Bottoni Modifica/Elimina (utenti, categorie, centri di costo) invisibili senza hover
...
### MEDIA-38 · `src/pages/MarginiCategoria.tsx:563` — Pie con raggio fisso 120px ed etichette esterne clippate su mobile
...
### MEDIA-39 · `src/pages/Onboarding.tsx:524` — Campi numerici senza inputMode/type dedicato: tastiera completa al posto del tastierino
...
### MEDIA-40 · `src/pages/PrimaNota.tsx:305` — Scroll trap: tabella con max-h-[70vh] e scroll proprio dentro la pagina scrollabile

*Area: Gesti e scroll · Verifica: manuale*

**Problema (mobile):** La tabella movimenti e' in `<div className="overflow-auto max-h-[70vh]">` dentro `<main>` che scrolla. Su smartphone il contenitore occupa quasi tutto il viewport: qualunque swipe verticale atterra sulla tabella e scrolla quella, non la pagina — per raggiungere ciò che sta sotto bisogna prima esaurire 200+ righe o mirare ai margini. Senza `overscroll-contain` il gesto inoltre "rimbalza" tra i due scroller a fine corsa. Stesso pattern in ContoEconomico.tsx:1852 (`max-h-[500px] overflow-y-auto` sull'anteprima PDF) e, in misura minore, Dashboard.tsx:756 (`max-h-[280px]`). [Verifica: PrimaNota e' viva come tab 'Prima nota' dentro Banche (TesoreriaManuale.tsx:3826).]

**Fix proposto:** Disattivare il doppio scroller su mobile: `md:max-h-[70vh] md:overflow-auto` (su telefono la tabella scorre con la pagina, lo sticky header continua a funzionare rispetto a <main>), oppure mantenere il max-h ma aggiungere `overscroll-contain` e ridurre l'altezza (`max-h-[50dvh]`) cosi' resta visibile contesto scrollabile della pagina.

### MEDIA-41 · `src/pages/PrimaNota.tsx:306` — Prima Nota: 8 colonne, a 360px visibili solo Data/Conto/Tipo
...
### MEDIA-42 · `src/pages/Produttivita.tsx:587` — Legend default di recharts con 7 serie outlet dentro altezza fissa del grafico
...
### MEDIA-43 · `src/pages/RevisionePagamenti.tsx:311` — Griglia di editing min-w-[820px] con select nelle celle: modifica impraticabile da touch
...
### MEDIA-46 · `src/pages/ScadenzarioSmart.tsx:1467` — Toast di errore che concatenano error.message tecnico grezzo (spesso in inglese)
...
### MEDIA-47 · `src/pages/ScadenzarioSmart.tsx:2614` — Card KPI tab Situazione in grid-cols-2 gap-6: importi text-2xl tagliati
...
### MEDIA-48 · `src/pages/ScadenzarioSmart.tsx:3603` — Badge informativi a 10px nella tabella principale dello scadenzario
...
### MEDIA-49 · `src/pages/ScadenzarioSmart.tsx:3869` — Tabelle Scadenzario senza paginazione + componente monolitico da 4.951 righe
...
### MEDIA-50 · `src/pages/ScadenzarioSmart.tsx:4830` — Pulsanti icona con touch target 26-30px (minimo raccomandato 44x44px)
...
### MEDIA-51 · `src/pages/ScadenzeFiscali.tsx:97` — Modal scadenza fiscale non chiudibile con tap sull'overlay né con Esc
...
### MEDIA-52 · `src/pages/ScadenzeFiscali.tsx:466` — Scadenze fiscali: 8 colonne con colonna Azioni w-36 irraggiungibile senza scroll cieco
...
### MEDIA-53 · `src/pages/ScadenzeFiscali.tsx:531` — Bottone azione 'Pagato' con testo 10px e touch target ~26px
...
### MEDIA-54 · `src/pages/TesoreriaManuale.tsx:140` — xlsx (~140KB gzip) importato staticamente in 7 file, incluso Banche (pagina quotidiana)
...
### MEDIA-55 · `src/pages/TesoreriaManuale.tsx:3236` — Tab Riconciliazione renderizza tutti i movimenti senza paginazione (fetch fino a 10.000 righe)
...
### MEDIA-56 · `src/pages/TesoreriaManuale.tsx:3530` — Testo a 9px: 'match importo' nella riconciliazione manuale e '+N' nel calendario
...
### MEDIA-57 · `src/pages/Ticket.tsx:459` — Modal 'Nuova segnalazione': items-center + overflow-y-auto rende la testata irraggiungibile
...
### MEDIA-58 · `src/pages/Ticket.tsx:1135` — URL Edge Function ticket-resolve-now letto da VITE_SUPABASE_URL senza resolver tenant

*Area: Coerenza multi-tenant · Verifica: manuale*

**Problema (mobile):** `const supabaseUrl = import.meta.env.VITE_SUPABASE_URL` senza fallback a getCurrentTenant(): sui site Made/Zago che definiscono solo le var suffissate (design di tenants.ts) il valore e' undefined e la fetch diventa `undefined/functions/v1/ticket-resolve-now` → il bottone AutoFix AI dei ticket fallisce sempre su Made/Zago. La pagina Segnalazioni e' proprio quella usata da Sabrina/Veronica da smartphone su tutti e 3 i tenant: il bottone funziona su NZ e si rompe sugli altri due, divergenza silenziosa (errore di rete generico). [Verifica: Reintegrato dopo doppio controllo su tenants.ts: il design prevede env var SUFFISSATE per tenant (VITE_SUPABASE_URL_MADE/_ZAGO, tenants.ts:37) risolte da getCurrentTenant(); leggere direttamente VITE_SUPABASE_URL diverge dal resolver e, se il site Made/Zago definisce solo la variante suffissata, la URL risulta undefined e ticket-resolve-now si rompe su quei tenant. Nessun hardcode qui, quindi media e non alta.]
...
### BASSA-6 · `src/components/Layout.tsx:273` — Tre polling paralleli ogni 60s senza pausa quando la tab non è visibile
...
### BASSA-7 · `src/components/Layout.tsx:320` — Touch target sotto i 44px per hamburger e X di chiusura drawer
...
### BASSA-8 · `src/components/Sidebar.tsx:453` — Disclosure e menu senza aria-expanded; bottoni icona con solo title
...
### BASSA-9 · `src/data/pageGuides.ts:115` — Guide utente e assistente AI usano 'Outlet' fisso, divergendo dai nomi pagina di Made/Zago
...
### BASSA-11 · `src/hooks/useCompanyLabels.ts:71` — Fallback terminologia divergente: 'Outlet' in useCompanyLabels vs 'Punto vendita' in useCompany
...
### BASSA-12 · `src/lib/tenants.ts:95` — Contrasto del TenantBadge insufficiente e diverso per tenant (peggiore su Zago)
...
### BASSA-13 · `src/pages/CashflowProspettico.tsx:1449` — Stati di loading centrati con h-screen/min-h-screen dentro il main scrollabile

*Area: Orientamento e viewport · Verifica: manuale*
...
### BASSA-14 · `src/pages/Dashboard.tsx:82` — Affordance "Dettaglio" delle KpiCard visibile solo su hover: invisibile al touch
...
### BASSA-15 · `src/pages/Fornitori.tsx:1312` — YAxis categoria con width fissa 120px: 40% dello schermo per etichette troncate

*Area: Grafici e dashboard · Verifica: avversariale*

**Problema (mobile):** Nel bar chart orizzontale "Top fornitori per spesa" la YAxis ha width={120} fisso: su un container mobile di ~300px riserva il 40% alle ragioni sociali, lasciando ~180px alle barre; al contempo 120px non bastano per nomi fornitore reali (>18 caratteri) che vengono clippati dall'SVG senza ellissi né title.

**Fix proposto:** Ridurre la width su mobile (es. 90) e applicare un tickFormatter che tronchi a ~14 caratteri con "…"; il nome completo resta consultabile nel GlassTooltip al tap sulla barra.
...
### BASSA-18 · `src/pages/ScadenzarioSmart.tsx:2711` — Barra filtri scadenzario: controlli text-xs py-1.5 (~30px) e nessuna priorita' mobile
...
### BASSA-19 · `src/pages/ScadenzarioSmart.tsx:3990` — Torta "Categoria" senza etichette né legenda: colori non decodificabili senza tap
...
### BASSA-20 · `src/pages/SchedaContabileFornitore.tsx:914` — Note esplicative critiche a 11px slate-400
...
## Finding scartati dalla verifica avversariale (9)
...
- `src/components/SupplierAllocationEditor.tsx:509` — *Righe split percentuale/valore: label fissa w-40 schiaccia l'input a ~50px su mobile* → **scartato**: Il codice alle righe 506-523 e 546-562 esiste come descritto, ma il problema non si verifica: SupplierAllocationEditor è renderizzato solo dentro il Pannello Gestione di Fornitori.tsx, in un td di una tabella con min-w-[920px] dentro overflow-x-auto (righe 848-849). Su viewport 360-430px la tabella mantiene 920px e scorre orizzontalmente, quindi ogni riga split dispone di ~850px: l'input conserva l'intera max-w-[140px] e non viene mai schiacciato a ~50px. Lo scroll orizzontale dell'intero pannello è un'altra questione, non quella descritta dal finding.
- `src/components/Sidebar.tsx:519` — *Drawer mobile: il tasto back di Android non lo chiude e manca Escape* → **scartato**: Il comportamento denunciato non esiste: l'effetto alle righe 283-285 chiude il drawer a ogni cambio di location.pathname, e il back hardware di Android passa da react-router (popstate), quindi il drawer si chiude automaticamente e non resta mai aperto sopra la nuova pagina. Il residuo (back che chiude senza navigare, Escape, focus trap) è una preferenza di pattern: Escape è irrilevante su touch e il drawer ha già chiusure funzionanti via X, overlay e navigazione.
- `src/components/ui/Breadcrumb.tsx:20` — *Componente ui/Breadcrumb mai usato e comunque non responsive* → **scartato**: Il componente src/components/ui/Breadcrumb.tsx e' effettivamente codice morto (esportato da ui/index.ts riga 4 ma nessun file importa dal barrel ne' dal componente; il Breadcrumb reale e' la funzione locale in Layout.tsx riga 76). Proprio per questo il finding non e' un problema mobile riproducibile: il componente non viene mai renderizzato, quindi l'overflow ipotizzato su 360px non puo' verificarsi nell'app reale. E' una segnalazione di pulizia codice, fuori scope per un audit di usabilita' mobile.
- `src/pages/Dashboard.tsx:904` — *Link 'Vedi dettaglio' outlet in classifica visibile solo su hover, riga non cliccabile* → **scartato**: Falso positivo: la tabella con il chevron hover-only (riga 904) e' dentro un wrapper 'hidden sm:block' (riga 861) e non esiste su viewport 360-430px. Su mobile c'e' una lista card dedicata 'sm:hidden' (righe 929-950) in cui ogni riga e' interamente un Link al dettaglio outlet con chevron sempre visibile (riga 948). Il pattern hover-only vale solo su desktop, dove l'hover e' disponibile. L'hint 'Dettaglio' della KPI card (riga 82) e' preferenza stilistica: la card intera e' gia' un Link tappabile.
- `src/components/HelpPanel.tsx:291` — *Pannello aiuto h-[70vh] + bottom-20: header tagliato in landscape e input chat sotto la tastiera* → **scartato**: In portrait 360-430px il pannello (70vh cappato a 560px + 80px) sta sempre dentro lo schermo: nessun taglio. Il taglio header riguarda solo il landscape (larghezza 640-930px, fuori scope), e anche lì il pannello resta chiudibile dal bottone flottante di toggle a bottom-6 right-6 sempre visibile (riga 279), quindi il claim "non si può chiudere" è falso. Per la tastiera: i browser fanno pan del visual viewport sull'input focussato (pattern standard dei widget fixed tipo Intercom) e il fix dvh proposto non avrebbe comunque effetto con il default Chrome interactive-widget=resizes-visual (nessun override nel meta viewport di index.html).
- `src/pages/Scadenzario.tsx:951` — *Errori mostrati con alert() nativo del browser invece del sistema toast* → **scartato**: Dead code: la route /scadenzario carica ScadenzarioSmart (App.tsx:14); nessun alert() presente nelle pagine effettivamente raggiungibili.
- `src/pages/CashFlow.tsx:400` — *Stati loading/errore/login di CashFlow in inglese con errore tecnico grezzo a schermo* → **scartato**: Dead code: la route /cash-flow carica CashflowProspettico (App.tsx:24); la pagina CashFlow.tsx con i testi in inglese non e' raggiungibile.
- `src/data/pageGuides.ts:2224` — *Guida Ticket: allegati spiegati solo con Ctrl+V/Cmd+V, nessuna alternativa da telefono* → **scartato**: La guida offre gia' alternative touch: 'trascinarli nell'area apposita' e 'cliccare per selezionarli' (che su telefono apre il file picker). Ctrl+V e' solo una delle tre opzioni.

---

*Report generato dall'audit multi-agente del 2026-07-19. Ogni finding confermato è stato verificato leggendo il codice citato. Nessuna modifica applicata al codice o ai dati: i fix sono proposte da implementare in PR separate.*
