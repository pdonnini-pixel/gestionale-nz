...
export const PAGE_GUIDES: PageGuide[] = [
  ...
  },
  {
    ...
    "sections": [
      ...
      {
        "heading": "Prima Nota",
        "body": "È una vista pronta per l'esportazione, da consegnare alla commercialista, costruita come un estratto conto: per ogni conto il saldo iniziale, i movimenti del periodo con il saldo progressivo dopo ognuno, il saldo finale calcolato e quello della banca. Mostra i movimenti bancari di un periodo (anno o singolo mese, tutti i conti o uno solo) con data, conto, tipo di movimento, importo, saldo progressivo, controparte, partita IVA, numero di fatture saldate, causale e categoria. Il filtro \"Periodo per\" decide quale data conta per stare nel periodo: \"Data contabile (banca)\", quella stampata dalla banca sull'estratto conto e che lo studio legge, oppure \"Data operazione\", quella in cui il movimento è avvenuto (di solito coincidono; a fine mese le competenze e le commissioni possono avere data contabile nel mese dopo). Quando le due date sono diverse la tabella mostra anche l'altra sotto la data principale, e nell'Excel ci sono entrambe le colonne. Il saldo progressivo è esatto a fine giornata; l'ordine dei movimenti dentro la stessa giornata è quello dell'elenco. Il \"Tipo movimento\" dice che cos'è ogni riga senza doverla leggere: Pagamento fornitore (ha una o più fatture agganciate), F24 / imposte, Stipendi, Incasso POS, Versamento contanti, Carta di credito (l'addebito mensile dell'estratto della carta di credito), Carta di debito (POS: un pagamento con la carta di debito, che esce subito dal conto), Ricarica carta prepagata (la ricarica della Tasca: non è una spesa, sono soldi passati dal conto alla carta, la controparte è «Carta prepagata Tasca» e le spese fatte con quei soldi stanno nella vista Carte), Finanziamento (rate di mutuo), Spese e commissioni bancarie (canoni, commissioni, fideiussioni, SDD Nexi e Global Blue), Giroconto / prelievo, oppure Da chiarire quando nessuna fonte lo spiega. Lo ricava prima dagli agganci (fattura in Scadenzario, scadenza in Scadenze fiscali), poi dalla causale scritta dalla banca, infine dall'etichetta della categoria. Un movimento che salda più fatture (una RiBa con trenta effetti, una distinta CBI) porta TUTTE le fatture: la controparte è il fornitore se è uno solo, altrimenti \"N fornitori (M fatture)\", la causale elenca tutti i numeri di fattura (con il fornitore accanto quando sono di fornitori diversi), la colonna Fatt. mostra quante sono e, al passaggio del mouse, il loro totale e la differenza con l'addebito (di solito la commissione). Le fatture a RICEVUTA BANCARIA si riconoscono senza aprire lo Scadenzario: se tutte le fatture del movimento sono RiBa il tipo diventa \"Pagamento fornitore (RiBa)\" e la causale comincia con \"RiBa · Fatt. …\"; se lo sono solo alcune il tipo dice \"(RiBa e altro)\" e la sigla RiBa compare accanto al numero della singola fattura. Il metodo è quello registrato sulla scadenza (RiBa 30/60/90/120 giorni). Per gli F24 agganciati in Scadenze fiscali la causale riporta titolo, codice tributo e periodo. Se non c'è nessun aggancio, la controparte viene letta dal beneficiario in causale (bonifici con il nome dopo l'asterisco, SDD \"a favore di\"). Per gli accrediti POS e Amex e per i versamenti di contante la controparte è il punto vendita di riferimento con il canale (per esempio \"PLM · Palmanova, POS MPS\"), attribuito come nella vista Incassi per outlet; se l'attribuzione manca resta il testo della banca. Un bonifico in entrata di un cliente privato per un acquisto (causale con \"acquisto\", \"abito\", \"merce\", \"ordine\") è un Incasso cliente (bonifico), cioè un corrispettivo pagato con bonifico: la controparte è l'ordinante; un bonifico in entrata con \"rimborso\", \"restituzione\" o \"storno\" è un Rimborso / restituzione. Per assegnare a mano a un outlet un incasso che il sistema non può attribuire (il bonifico di un cliente per un acquisto in un negozio preciso), si scrive nella nota del movimento, in Banche → Movimenti, \"Outlet: CODICE · motivo\" (per esempio \"Outlet: BRB · corrispettivi Barberino agosto 2026\"): la Prima Nota e la vista Incassi per outlet leggono quel codice e mostrano l'outlet con attribuzione \"Assegnato a mano (nota)\". In alto trovi i totali del periodo (movimenti, entrate, uscite, saldo netto) e il contatore \"Da chiarire\", poi una riga con quante righe e quanti euro ci sono per ogni tipo di movimento. Sotto, il riquadro \"Quadratura con l'estratto conto\", per la data scelta nel filtro \"Periodo per\": per ogni conto il saldo al giorno prima del periodo (per agosto, al 31/07), le entrate e le uscite del periodo, il saldo all'ultimo giorno calcolato (saldo iniziale più movimenti), il saldo all'ultimo giorno secondo la banca e la differenza. I saldi vengono dallo scarico A-Cube (il saldo del conto al momento dello scarico), non dai movimenti: se la differenza è zero l'estratto conto è completo, senza movimenti mancanti né doppioni. Siccome lo scarico non cade sempre sul confine del mese, il saldo alla data è il saldo letto dalla banca allo scarico, rettificato: più i movimenti datati entro quel giorno ma arrivati dopo lo scarico, meno i movimenti già nello scarico ma datati dopo (per esempio una competenza con data contabile nel mese successivo). Sotto ogni saldo vedi quando la banca l'ha fornito e quante rettifiche ci sono, e al passaggio del mouse la composizione. La tabella dei movimenti ripete la stessa struttura conto per conto: una riga di apertura con il saldo iniziale, i movimenti con il saldo progressivo, una riga di chiusura con saldo finale calcolato, saldo della banca ed esito; con un filtro per tipo attivo l'elenco è parziale e i saldi non vengono mostrati. Per i mesi importati prima di marzo 2026 la banca non ha fornito i saldi e la riga dice \"saldi n.d.\". Nello stesso riquadro c'è il Contante: versamenti di contante in banca e prelievi dalla banca, e, se nel periodo ci sono chiusure di cassa (dal 1° settembre 2026), contanti incassati nei negozi, spese di cassa, rimborsi, versamenti dichiarati nelle chiusure e quanti sono stati ritrovati in banca, fondo cassa più da versare a inizio e fine periodo (contato e calcolato) e la differenza cassa; se il primo giorno di un outlet parte senza fondo noto, la quadratura cassa comincia dal periodo successivo. La card \"Da chiarire\" e ogni etichetta della riga per tipo si cliccano: la tabella mostra solo quei movimenti, un avviso arancione dice quanti sono e come sistemarli (fornitore in Riconciliazione, F24 in Scadenze fiscali), e \"Togli filtro\" riporta l'elenco completo; il filtro si azzera da solo cambiando periodo, conto o vista, e gli export CSV/Excel restano sempre completi. Sopra la tabella scegli la vista: \"Movimenti banca\" (una riga per movimento, quella descritta fin qui), \"Pagamenti fornitori\" (una riga per FATTURA pagata nel periodo, secondo la data di pagamento dello Scadenzario), \"Incassi per outlet\" (le entrate del periodo attribuite al punto vendita) \"Dipendenti\" (una riga per busta paga, con la disposizione bancaria che l'ha pagata) oppure \"Carte\" (gli estratti conto delle carte del periodo, riga per riga, come per le banche). Nella vista Pagamenti fornitori una RiBa da trenta effetti diventa trenta righe: per ognuna vedi quando è stata pagata, la Fonte (Banca se c'è il movimento riscontrato, Contanti, Carta, Nota di credito per le note compensate, Provvisoria se il pagamento è dichiarato ma senza riscontro, Chiusa a mano, Senza riscontro), il conto e la data del movimento bancario, fornitore e partita IVA, numero e data fattura, la rata se il piano ne ha più di una, imponibile, IVA, eventuale ritenuta, lordo, importo pagato, metodo, categoria di costo e conto del conto economico, outlet se assegnato; la motivazione di una chiusura a mano compare passando il mouse sul badge Fonte. Entrano anche le fatture pagate in contanti o con carta, che non passano dal conto e non compaiono fra i movimenti; con un conto selezionato restano solo le fatture riscontrate o disposte su quel conto. Le card mostrano fatture pagate, importo, fornitori e il contatore \"Senza riscontro\" (dichiarate pagate senza movimento né contanti/carta: da agganciare in Riconciliazione); sotto, una riga con righe e importi per fonte. Anche qui la card \"Senza riscontro\" e le etichette per fonte si cliccano per vedere solo quelle fatture, con \"Togli filtro\" per tornare all'elenco completo. Nel foglio Excel «Incassi per outlet» le righe sono raggruppate per conto e, dentro ogni conto, per data: la prima riga \u00e8 il primo incasso del primo conto, non del primo giorno del mese (il Banco Fiorentino raccoglie solo i versamenti di contante, che partono pi\u00f9 tardi). Una riga di intestazione lo dice anche nel file. Nella vista Incassi per outlet ogni entrata in banca (giroconti e rimborsi esclusi: la restituzione di un fornitore o la liquidazione transattiva di un corriere non è un incasso e resta solo nella vista Movimenti banca come Rimborso / restituzione) è attribuita al punto vendita senza chiedere niente: prima dall'abbinamento con la chiusura di cassa fatto dal riscontro notturno (POS, Amex, versamenti e, dal 16 settembre 2026, bonifici dei clienti sulla riga «Bonifico» della chiusura), poi dal codice terminale scritto nella causale dell'accredito (MPS, Amex, BCC/Numia/PagoBancomat) confrontato con i canali POS dell'outlet in Incassi giornalieri → Canali (stesso codice, stesso tipo POS o Amex e, a parità, stesso conto), infine, per i versamenti di contante, dalla parola chiave del canale Contanti (PALMANOVA, FOIANO, ATM 9750, cassa continua). Per ogni riga vedi data dell'operazione e giorno di vendita a cui si riferisce, conto, outlet e canale, Tipo (POS, Amex, Versamento contanti, Bonifico cliente per i bonifici di clienti, Altro incasso per gli altri accrediti), importo, il badge di attribuzione (Chiusura di cassa, Codice terminale, Parola chiave versamento, Da attribuire) e causale. Nel foglio Excel e nel CSV le colonne sono solo quelle che servono allo studio: Data operazione, Data riferimento, Conto Banca, Outlet, Canale, Tipo, Terminale, Importo, Causale (niente IBAN, attribuzione né categoria). Le card mostrano il totale incassato in banca, POS e Amex, versamenti, altri incassi e il contatore \"Da attribuire\"; sotto, una tabella per outlet con movimenti, POS, Amex, versamenti, altro e totale. La card \"Da attribuire\" e il nome di ogni outlet nella tabella si cliccano per vedere solo quelle entrate, con \"Togli filtro\" per tornare all'elenco completo: un accredito POS senza outlet ha un codice terminale non censito nei canali, un versamento senza outlet non contiene la parola chiave del canale Contanti, e si sistemano in Incassi giornalieri → Canali; un bonifico di cliente prende l'outlet dalla riga «Bonifico» della chiusura di cassa (il riscontro notturno abbina il bonifico in banca, o due bonifici che sommano, alla riga della chiusura con lo stesso importo fra dieci giorni prima e tre dopo): se resta senza outlet, il negozio non l'ha registrato nella chiusura. Nella vista Dipendenti ogni disposizione per emolumenti del periodo (i movimenti di tipo Stipendi, di solito il 10 del mese, una per outlet) viene abbinata alle buste paga caricate in Dipendenti → Costo lordo: il sistema legge dalla causale della banca l'ID flusso CBI, il numero di pagamenti, l'importo dei bonifici e le commissioni, e cerca fra i netti del mese prima (o dello stesso mese, per gli anticipi), raggruppati per outlet, il gruppo che somma esattamente all'importo dei bonifici; se un outlet è pagato con due disposizioni (per esempio la sede), trova il sottoinsieme giusto. Per ogni riga vedi nome e cognome, outlet, mese di competenza, netto, data del pagamento, conto, ID flusso (con la causale completa al passaggio del mouse), quanti bonifici la banca conta nel flusso e quante buste lo spiegano, commissioni e importo del flusso, l'ADDEBITO IN BANCA (importo del flusso più commissioni, cioè la cifra esatta del movimento sull'estratto conto: chi tiene la contabilità la cerca in banca per assegnare la distinta a quel gruppo di dipendenti) ed esito. Se i bonifici contati dalla banca sono più delle buste (la sede: una busta da 10.959,00 pagata con quattro bonifici, 5,00 di commissioni), il netto è stato versato in più bonifici e l'esito lo dice in chiaro. Le card mostrano quante disposizioni ci sono (con bonifici e commissioni), i netti pagati, le buste nel foglio, le buste del mese prima senza pagamento nel periodo e le disposizioni che nessun gruppo di buste spiega al centesimo (buste non ancora importate o importo diverso: da guardare con lo studio paghe), elencate in un riquadro arancione. Non c'è niente da compilare: se i netti sono caricati, l'abbinamento è automatico. Nella vista Carte ogni estratto conto carta del periodo (carte di credito CartaBCC/Numia e Carta Montepaschi, prepagata Tasca) è un blocco con le sue operazioni: data di acquisto e di registrazione, descrizione, importo (spese in rosso, ricariche e storni in verde), commissioni, la fattura dello Scadenzario pagata con quella riga (stesso importo, pagata con carta entro dieci giorni; se nella descrizione compare il nome del fornitore la ricerca arriva a 45 giorni, se compare il numero della fattura a quattro mesi, perche\u0301 lo stesso fornitore puo\u0301 ripetere lo stesso importo in mesi diversi) e il riscontro con la banca. Per le carte di credito il riscontro è l'addebito unico dell'estratto sul conto, cercato per importo nei 75 giorni successivi: la BCC addebita le due carte in un movimento solo più le commissioni, quindi l'addebito vale per tutti gli estratti dello stesso mese e la differenza mostrata sono le commissioni; per la prepagata il riscontro è, ricarica per ricarica, l'addebito \"Ricarica carta\" in banca. I colori della colonna Riscontro banca: verde quando il riscontro c'è (estratto addebitato il…, addebito in banca il…), arancione quando manca qualcosa che doveva esserci (ricarica non trovata in banca), grigio quando non c'è niente da cercare o non è ancora dovuto («spesa sulla carta: non passa dal conto» per le spese della prepagata, «in attesa dell'addebito» per un estratto di carta di credito il cui addebito, di solito il 25 del mese dopo, non è ancora arrivato). Nella tabella della prepagata, a parità di giorno, le ricariche vengono prima delle spese: il saldo progressivo non scende mai sotto zero. La PREPAGATA (Tasca) ha anche il SALDO: nell'intestazione del blocco vedi saldo iniziale e saldo finale del mese, e la tabella, in ordine cronologico, ha la colonna Saldo con il saldo della carta dopo ogni riga (le ricariche lo alzano, spese e commissioni lo abbassano). Il portale Tasca non stampa un saldo: il PDF riporta solo la «Disponibilità» alla data di stampa, che il gestionale salva con l'estratto e usa come àncora. Il primo estratto che la dichiara ha quel saldo finale («dal documento»); i mesi prima e dopo si ricostruiscono a catena sommando o togliendo il netto di ogni mese («a catena»), leggendo tutti gli estratti della carta e non solo quelli del periodo. Se nessun estratto la dichiara, la catena parte da zero al primo estratto e il badge lo dice («da zero»): importare un PDF del portale basta ad ancorare tutto. La card «Ricariche e storni» riporta il saldo della prepagata a fine periodo. Il lettore del PDF Tasca ricompone i movimenti che il portale spezza su più righe (date e inizio descrizione, importi, orario e resto della descrizione). Gli estratti si importano con \"Importa estratto carta\" (PDF di CartaBCC e Carta Montepaschi, Excel o PDF del portale Tasca): il sistema legge dal documento la carta, il mese e le righe, mostra un'anteprima con totale letto, totale dichiarato dal documento ed eventuali avvisi, poi salva le righe e archivia il file in Banche → Archivio; un estratto già importato con le righe e con il documento in Archivio non viene toccato (il file viene saltato, e la finestra lo dice); se invece l'estratto ha le righe ma non il documento, l'import allega il file all'estratto senza toccare le righe (messaggio verde «questo file viene allegato all'estratto», pulsante «Allega N file»). Gli estratti già in Archivio ma senza righe (quelli caricati prima di questa funzione) hanno il pulsante \"Leggi le righe dal file archiviato\", che fa la stessa cosa senza ricaricare il file. Il caso opposto, un estratto con le righe ma senza documento in Archivio (per esempio caricato direttamente nel database), ha il pulsante «Allega il PDF»: scegli il file dell'estratto e viene archiviato e agganciato all'estratto, senza toccare le righe. Le card mostrano quanti estratti ci sono nel periodo (e quanti senza righe), le spese con carta, ricariche e storni, gli addebiti trovati in banca, le fatture agganciate e le spese con le carte di debito. Sotto gli estratti, un blocco per ogni CARTA DI DEBITO (BCC «Carta *453», le carte Montepaschi con il loro numero): la carta di debito non ha un estratto a parte, ogni pagamento POS esce subito dal conto, quindi il gestionale legge i pagamenti del periodo dai movimenti bancari (causale «Operazione POS … Carta *…» o «PAG.POS … N.CARTA: …»), ne ricava data di acquisto, esercente e commissioni, aggancia la fattura dello Scadenzario pagata con carta di debito (stesso importo entro dieci giorni, fino a 45 se l\u0027esercente in causale porta il nome del fornitore) e mostra come riscontro la data del movimento in banca. Nella vista Movimenti banca quei pagamenti hanno il tipo «Carta di debito (POS)». Da qui scarichi CSV (della vista aperta, con tutte le colonne e il saldo progressivo) o Excel. Il file scaricato porta in chiaro azienda, contenuto, periodo e giorno di esportazione, per esempio «New Zago Prima nota Agosto 2026 - esportato il 18-09-2026.xlsx»: cos\u00ec nella cartella di chi li archivia un mese non si confonde con l\u0027altro, n\u00e9 una versione con quella rifatta il giorno dopo. I due pulsanti restano spenti mentre i dati del periodo si stanno caricando e finché una parte non si \u00e8 caricata (compare il riquadro rosso «Errore caricamento»): passandoci sopra il messaggio dice se aspettare o ricaricare. \u00c8 una tutela: un file con l\u0027intestazione del mese giusto ma con dentro i movimenti di un altro periodo sarebbe indistinguibile da quello buono una volta inviato. L'Excel si apre sul foglio \"Guida\": spiega foglio per foglio cosa c'è, cosa vuol dire ogni colonna, la legenda dei colori e come si cerca (un importo con Ctrl+F scritto come appare, un fornitore o una fattura col filtro, la distinta stipendi partendo dall'addebito in banca). Il file ha una grafica fissa: intestazioni in neretto bianco su blu, bloccate in alto e con il filtro automatico; righe di saldo iniziale e finale in neretto su grigio; movimenti e fatture a ricevuta bancaria su fondo ambra; dettaglio fattura sotto il movimento in corsivo grigio; riga della differenza verde se quadra, rossa se NON QUADRA; righe da guardare (incasso senza outlet, busta senza pagamento, disposizione senza buste) su fondo ambra; linguette colorate per tipo di foglio. Poi l'Excel ha un foglio per ogni conto, impaginato come un estratto conto: intestazione con banca, IBAN, periodo e data usata, riga del saldo iniziale (con il saldo letto dalla banca e le rettifiche), i movimenti con data operazione, data contabile, tipo, controparte, partita IVA, fatture, causale, categoria, entrate, uscite e saldo progressivo; quando un movimento salda più fatture (una RiBa, una distinta), sotto di esso c'è una riga per ogni fattura con fornitore, partita IVA, numero, data, rata e importo nella colonna \"Di cui fattura\" (per le fatture a ricevuta bancaria la riga si chiama \"di cui fattura RiBa\" e la sigla RiBa è ripetuta accanto al numero), più la riga del resto (commissioni, acconto, nota di credito) se la somma delle fatture non coincide con l'uscita, e la riga del totale; sotto ogni «Ricarica carta prepagata» una riga «↳ ricarica prepagata» rimanda al foglio della carta dove quei soldi sono stati spesi, con il numero di spese del mese e il saldo della carta a fine mese (ambra se l'estratto della carta non è stato importato); poi le righe del saldo finale calcolato, del saldo finale della banca e della differenza. Seguono i fogli Incassi per outlet (una riga per entrata, con titolo e periodo in testa), Codici negozi (una riga per punto vendita con i codici con cui compare sui documenti: punto vendita e contratto Nexi con il regime di accredito, codici esercente Amex, numeri di terminale POS per banca, parola chiave dei versamenti; in coda, dove si legge ogni codice sull'estratto Nexi, sull'estratto Amex e nelle causali della banca, perché gli estratti Nexi e Amex non riportano il nome del negozio), Dipendenti ed emolumenti (una riga per busta paga con netto, disposizione che l'ha pagata, importo del flusso, commissioni e addebito in banca, e in coda le disposizioni senza buste con le stesse colonne) e un foglio per ogni estratto carta con righe, totale letto e dichiarato, addebito in banca e differenza (per la prepagata: in testa la riga «Come si riconcilia», che spiega che le ricariche sono le uscite del foglio del conto e le spese sono le righe negative, poi riga del saldo iniziale, righe in ordine cronologico con la colonna Saldo, saldo finale calcolato, ancoraggio del saldo e ricariche ritrovate in banca), più un foglio per ogni carta di debito con i pagamenti POS del periodo (esercente, importo, fattura pagata, riscontro in banca) e il totale. Tutti gli importi sono in formato euro. Le viste Movimenti banca (tabella piatta) e Pagamenti fornitori e i riepiloghi per tipo, per fonte e per outlet restano a video e nel CSV, non nell'Excel.",
        "steps": [
      ...
      },
      {
        ...
        "steps": [
          ...
          "Clicca «Carica estratti» e seleziona i PDF ricevuti da Amex o da Nexi, oppure direttamente lo zip che li contiene tutti: non serve estrarlo prima.",
          "Leggi l'esito riga per riga: dice da quale zip veniva il file, quale mese \u00e8 stato aggiornato, quanti punti vendita e con quale nome il documento \u00e8 stato archiviato. Se compare «Codici non censiti», quel codice non \u00e8 ancora collegato a un punto vendita e va aggiunto prima di ricaricare il documento.",
  ...
  },
  {
    ...
    "icon": "Users",
    "title": "Dipendenti",
    "description": "La pagina Dipendenti raccoglie l'anagrafica dei dipendenti, i cedolini mensili (netto e costo lordo), il confronto con il budget del personale e i ratei di ferie e permessi letti dalle paghe, tutto suddiviso per punto vendita. È il punto dove tenere aggiornati organico, stipendi, allocazioni e residui di ferie.",
    "sections": [
      {
      ...
      {
        "heading": "Ferie e permessi: le richieste",
        "body": "È la scheda dove si registra chi chiede ferie o permessi, e quanto gliene resta davvero. Si comincia scegliendo la persona: accanto al nome compaiono il suo punto vendita, l'orario settimanale e quanto vale per lei una giornata intera, perché il conto è in ore e una giornata non vale otto ore per tutti. Sotto ci sono tre riquadri, uno per voce (ferie, permessi ex festività, ROL): dicono quante ore sono disponibili oggi, quante ne risulteranno in totale a fine anno contando quello che deve ancora maturare, quante sono già impegnate da altre richieste e quante ne toglie quella che stai scrivendo. Il totale a fine anno non è una scadenza: le ferie non scadono, e quelle non godute restano. I saldi arrivano dal tabulato delle paghe della scheda «Saldi dalle paghe»: senza tabulato, quella persona non ha numeri e il riquadro lo dice invece di inventarli. IL CALENDARIO funziona come un pennello: in alto scegli la voce (Ferie, Ex festività, ROL) e il tipo (Giornata, Mezza, Ore, e in questo caso quante ore), poi clicchi i giorni. Un altro clic sullo stesso giorno lo toglie. Lo stesso giorno può portare due voci diverse, per esempio mezza giornata di ferie e mezza di ROL, ma il totale non può superare la giornata. Domeniche e festività nazionali (Pasqua compresa) restano grigie: si possono segnare lo stesso, ma il gestionale avvisa che di solito in un giorno non lavorativo le ferie non si consumano. CHI ALTRO È GIÀ VIA si vede in due posti, senza doverlo chiedere a nessuno. Sul calendario, accanto al numero del giorno, compare un numero arancione con l'omino: dice quanti colleghi dello stesso punto vendita hanno già ferie o permessi quel giorno, e si vede prima di cliccare. Sotto l'elenco dei giorni scelti c'è il riquadro con i nomi, giorno per giorno, e accanto a ciascuno la voce (Ferie, Ex festività, ROL) e, quando la richiesta non è ancora stata approvata, la scritta «da decidere». Le bozze non compaiono, come non impegnano ore: non sono ancora la richiesta di nessuno. Il punto vendita è quello dell'anagrafica, nella scheda Organico: chi non ce l'ha non può vedere i colleghi, e il riquadro lo dice. Sono ferie e permessi registrati qui dentro, non i turni: chi è di turno quel giorno, e le assenze di altro tipo (malattia, maternità, infortunio), il gestionale non le conosce e non le conta. GLI AVVISI sotto l'elenco dei giorni sono di tre tipi: in rosso quelli che fermano il salvataggio (più di una giornata nello stesso giorno, oppure più ore di quante ne avrà in tutto a fine anno), in giallo quelli che lasciano passare ma vanno letti (chiede più ore di quelle disponibili oggi, oppure un giorno festivo), in grigio le informazioni, come i giorni precedenti alla data del tabulato: quelli le paghe li hanno già contati e non tolgono altre ore al saldo. SI SALVA in due modi: «Salva come bozza» tiene la richiesta qui senza impegnare ore, «Registra la richiesta» la mette in attesa di risposta e le ore risultano impegnate. In entrambi i casi si indica come è arrivata (modulo cartaceo, mail, o compilata insieme alla persona), perché finché i dipendenti non hanno un accesso al gestionale il giro si fa ancora a mano. I MODULI DA FAR COMPILARE sono il riquadro in fondo: generano un foglio per persona, in PDF o in Excel, con il suo saldo già stampato sopra e le righe da riempire a penna, più lo spazio per le due firme. Sul foglio il saldo è scritto in ORE, con i decimali e la parola per esteso («0,55 ore», «11,20 ore»), su due righe sole: Ferie e Permessi, con ex festività e ROL sommati in un'unica voce. Le ore sono scritte come le scrive lo studio paghe, così chi ha in mano il tabulato ritrova lo stesso numero riga per riga: niente minuti, che sul documento non compaiono. Quale delle due borse si scala lo decide l'ufficio quando registra la richiesta, e comunque la decisione finale è del referente. PERCHÉ NON IN GIORNATE: perché non sappiamo quanti giorni a settimana lavora davvero ciascuno. Le paghe spalmano l'orario su cinque giorni per fare i conti, quindi per chi ha 8 ore settimanali una «giornata» contabile dura 1 ora e 36 minuti; se quella persona in negozio ci va un giorno solo e fa 8 ore di fila, quella giornata non esiste e scriverle «hai 5 giornate» quando ne ha una sarebbe falso. Il dato dei giorni lavorati non sta né sul tabulato delle paghe né in anagrafica: lo sa chi fa i turni. Appena ci sarà, il foglio tornerà a parlare di giornate, e saranno giornate vere. Si può generare solo per la persona scelta oppure per tutti quelli in forza in una volta sola, e in quel caso esce un file solo con una pagina (o un foglio Excel) a testa. Il foglio è scritto per chi lo riceve: ci sono le sue ore e il giorno a cui sono aggiornate, non da dove il gestionale le ha prese. Quello che riguarda noi, cioè il tabulato delle paghe da cui arrivano i saldi e come sono stati ricavati, resta su questa pagina e non finisce sul modulo. Quando la richiesta è già scritta qui dentro, gli stessi pulsanti PDF ed Excel producono il riepilogo di quello che è stato chiesto, con i giorni raggruppati per periodo continuo. L'ELENCO IN FONDO mostra le richieste già registrate, con lo stato, il periodo, le ore per voce e da dove sono arrivate. Una bozza si riprende e si modifica; una richiesta già inviata non si cancella, si ritira, e resta negli archivi con la sua storia: chi l'ha compilata, quando, e ogni cambio di stato."
      },
      {
        "heading": "Ferie e permessi: le approvazioni",
        "body": "È la scheda dove si risponde alle richieste. In cima ci sono quelle in attesa: si apre una riga e compaiono tutti i giorni chiesti, già tutti in verde, cioè concessi. Chi decide toglie quelli che non si possono dare, con un clic sul giorno, e il pulsante in fondo cambia di conseguenza: «Approva tutto», «Concedi 3 su 5», «Respingi tutto». SOTTO I GIORNI c'è chi altro è già via: i colleghi dello stesso punto vendita che hanno ferie o permessi negli stessi giorni, con il nome, la voce e la scritta «da decidere» quando la loro richiesta è ancora in attesa. Serve a non concedere due persone fuori lo stesso giorno senza saperlo. Sono ferie e permessi registrati qui dentro: i turni e le altre assenze (malattia, maternità) il gestionale non le conosce. IL MOTIVO È OBBLIGATORIO appena si toglie anche un solo giorno, e non è una formalità: chi riceve la risposta deve capire perché senza telefonare a nessuno. Il gestionale non lascia salvare finché non è scritto. Sotto, «Richieste già decise» mostra le ultime venti con l'esito, quanti giorni sono stati concessi su quanti chiesti, chi ha deciso e quando, e il motivo quando c'è. Una richiesta già decisa si può riaprire e decidere di nuovo: anche il ripensamento resta scritto. CHI APPROVA lo decide il riquadro in fondo alla scheda, e non dipende dal ruolo nel gestionale: si aggiunge una persona che ha già un accesso, e allora decide da qui, oppure solo un nome e un indirizzo, e allora riceve soltanto l'avviso per mail. Serve perché i referenti veri non hanno tutti lo stesso ruolo: chi è in sola lettura per il resto del gestionale può comunque approvare le ferie se è in questo elenco, e chi non ha un accesso riceve almeno la mail. Si può legare un referente a un solo punto vendita: in quel caso riceve gli avvisi delle persone di quel negozio. Chi gestisce il personale (super advisor, contabile, direzione operativa) decide comunque, anche senza essere nell'elenco. Un referente non si cancella: si toglie, e si può rimettere. LE MAIL partono in due momenti, quando una richiesta viene registrata e quando viene decisa, e vanno ai referenti che hanno un indirizzo. Se nessuno ce l'ha, non parte niente e la richiesta vale lo stesso: dentro il gestionale l'avviso c'è sempre, nella campanella delle notifiche."
      },
      {
        "heading": "Ferie e permessi: i saldi dalle paghe",
        "body": "È la scheda dove entra nel gestionale il residuo vero di ferie e permessi di ogni persona. Il gestionale non calcola la maturazione: legge la stampa «Situazione ratei di ferie e permessi» che lo studio paghe pubblica ogni mese, la stessa che arriva insieme ai cedolini. Il PDF si trascina nel riquadro in cima alla scheda, oppure si sceglie con «Carica il tabulato». Prima di salvare vedi un'anteprima con l'azienda, il periodo di elaborazione, quante persone sono state lette e quante sono state riconosciute nell'anagrafica. Un riquadro verde dice che la lettura è completa, cioè che le somme delle righe lette coincidono con i totali dell'azienda stampati in fondo al documento; se qualcosa non torna il riquadro diventa arancione ed elenca cosa. Per ogni persona la tabella mostra le tre voci che le paghe tengono separate (ferie, permessi ex festività e permessi ROL), tutte in ore, più l'orario settimanale del contratto e quanto vale una giornata piena, ricavati dal rateo annuo. Per i contratti a chiamata l'orario non viene ricavato: in quel caso il programma delle paghe fa maturare il rateo per intero, perché non può stimare la maturazione, e dividerlo darebbe un orario che non esiste. Un residuo negativo, cioè ferie godute in anticipo, è scritto in rosso. Chi risulta cessato nella stampa viene segnalato accanto al nome. L'abbinamento fra la riga del tabulato e la persona in anagrafica lo fa il sistema: usa il nome insieme alla matricola o alla data di assunzione, perché la matricola da sola non basta (fra gestionale e paghe può essere diversa). LE MATRICOLE SI ALLINEANO DA SOLE: quando quella del documento è diversa da quella in anagrafica, salvando viene riscritta con quella dello studio, che non può cambiare le sue. L'anteprima elenca prima quali stanno per cambiare, e la matricola vecchia resta nel registro delle matricole, così i dati già agganciati non si perdono. Solo le righe che il documento non basta a decidere restano da scegliere a mano, su fondo ambra: il selettore cerca per cognome o per matricola, così non si scorre l'elenco di tutti. Salvando, il file finisce in Archivio nella sezione «Paghe e personale», i saldi di quel mese diventano quelli correnti e l'orario settimanale letto dalle paghe viene scritto accanto a quello inserito a mano, senza sostituirlo. LE CESSAZIONI ARRIVANO DA SOLE: il tabulato scrive accanto a ogni nome la data di cessazione quando c'è, e se in anagrafica quella persona risulta ancora in forza viene segnata cessata con quella data. L'anteprima lo dice prima di salvare, con i nomi e le date, così si sa cosa sta per cambiare. Chi viene cessato resta in archivio fra i «Cessati» con tutta la sua storia e si può riattivare. Una data di cessazione già scritta a mano non viene mai sostituita. Reimportare lo stesso mese non cancella niente: il tabulato precedente resta in archivio e smette semplicemente di essere quello corrente. In fondo alla scheda c'è l'elenco dei tabulati già importati: si popola al primo salvataggio, perché caricare il file e guardare l'anteprima non salva niente. Per ogni tabulato vedi il mese, l'azienda, il giorno in cui è stato caricato, quante persone contiene e quante sono state riconosciute in anagrafica, e se è stato letto per intero, cioè se i totali stampati in fondo al documento delle paghe coincidono con la somma di quello che il gestionale ha letto. «Apri il documento» apre il PDF, che resta in Archivio nella sezione Paghe e personale. Cliccando sul mese il tabulato si riapre e mostra la stessa tabella dell'anteprima, persona per persona, con i residui salvati: chi è rimasto senza dipendente abbinato è su fondo ambra e si sistema ricaricando il tabulato e scegliendo la persona."
      }
    ],
    "faq": [
      {
        "q": "Un dipendente a chiamata non vede nessun saldo ferie: è un errore?",
        "a": "No, è corretto. Nel contratto a chiamata ferie e permessi vengono pagati ogni mese in busta paga, quindi il conteggio si azzera e non resta niente da godere: la scheda lo scrive al posto dei riquadri dei saldi, e il calendario non si apre. Sul tabulato delle paghe queste persone hanno un rateo pieno, 168 ore come un tempo pieno, perché il programma non può stimare la maturazione di chi lavora a chiamata: per questo motivo il gestionale non ricava il loro orario settimanale da quel numero, che darebbe quasi 39 ore a settimana per chi magari ne lavora cinque."
      },
      {
        "q": "Devo dire io al gestionale chi è stato cessato?",
        "a": "No, se la persona compare nel tabulato delle paghe: quel documento porta la data di cessazione accanto al nome, e caricandolo l'anagrafica si allinea da sola. L'anteprima elenca chi sta per essere segnato cessato, con la data, prima che tu salvi. Resta da fare a mano solo per chi nel tabulato non c'è, per esempio chi se ne va a inizio mese prima dell'elaborazione."
      },
      {
        "q": "Perché sul modulo del dipendente non ci sono le giornate?",
        "a": "Sul modulo le ore sono scritte per esteso («0,55 ore») e le voci sono due, Ferie e Permessi; qui restano con la virgola e divise in tre, perché servono a far tornare i conti. Le giornate sul modulo non ci sono, e non è una dimenticanza: la «giornata» che usano le paghe è l'orario settimanale diviso cinque, e per un part time corto non corrisponde a nessun giorno di negozio. Chi fa 8 ore a settimana ha una giornata contabile da 1 ora e 36 minuti, ma se lavora un giorno solo da 8 ore, il suo giorno libero costa 8 ore. Finché non sappiamo quanti giorni a settimana lavora ciascuno, dire giornate sarebbe inventare."
      },
      {
        "q": "Le ferie scadono a fine anno?",
        "a": "No. Le ferie non godute restano e si sommano a quelle nuove: lo ha confermato lo studio paghe. Il numero «Totale a fine anno» che vedi nei riquadri non è un termine ultimo, è solo quanto ne avrà in tutto quella persona una volta maturato tutto l'anno, cioè quello che ha oggi più quello che deve ancora maturare. Sul tabulato delle paghe esistono due colonne che sembrano parlare di scadenze («non indennizzabili» e «da godere nell'anno»): sono contatori interni del programma dello studio, il gestionale li conserva ma non li usa e non li mostra."
      },
      {
        "q": "Perché la matricola del gestionale non coincide con quella delle paghe?",
        "a": "Non dovrebbe più succedere: da settembre 2026 è il gestionale ad allinearsi allo studio. Lo studio assegna le matricole in automatico, in ordine cronologico di assunzione, e non può cambiarle; il gestionale invece sì, quindi quando carichi il tabulato dei ratei le matricole diverse vengono riscritte con quelle del documento. L'anteprima ti dice prima quali stanno per cambiare, e la matricola vecchia resta nel registro delle matricole, così i dati già agganciati (cedolini, costi, tabulati precedenti) non si perdono. Resta vero che una persona può averne avute più di una: chi viene riassunto con un contratto nuovo ne prende una nuova, ed è per questo che il riconoscimento usa il nome insieme alla matricola o alla data di assunzione."
      },
      {
        "q": "Il gestionale mi dice quante persone restano in negozio quel giorno?",
        "a": "No, e non è una mancanza: dice chi è già via, che è un fatto che conosce, e non chi è presente, che non conosce. Le ferie e i permessi registrati qui dentro li sa tutti; i turni no, e nemmeno le malattie, le maternità o gli infortuni. Contare i presenti sarebbe un numero sbagliato scritto con sicurezza. Sul calendario il numero arancione conta le persone dello stesso punto vendita che quel giorno hanno ferie o permessi, e sotto i giorni scelti trovi i loro nomi."
      },
      {
        "q": "Denise è in sola lettura: come fa ad approvare le ferie?",
        "a": "Perché l'approvazione non dipende dal ruolo ma dall'elenco dei referenti, nella scheda Approvazioni. Chi è in quell'elenco decide sulle richieste di ferie anche se per il resto del gestionale può solo guardare. Chi non ha proprio un accesso si può aggiungere con nome e indirizzo: non decide, ma riceve l'avviso per mail."
      },
      {
        "q": "Perché il gestionale dice che restano meno ore di quelle che vedo sul tabulato delle paghe?",
        "a": "Perché al residuo stampato sul tabulato toglie quello che è già stato chiesto qui dentro e non è ancora passato dalle paghe: le richieste inviate e quelle approvate. Nei riquadri dei saldi la riga \"Già in attesa\" dice quante ore sono impegnate così. Le bozze invece non tolgono niente finché non le registri."
      },
      {
        "q": "Ho segnato dei giorni di agosto e il saldo non è cambiato: è un errore?",
        "a": "No. I giorni precedenti alla data dell'ultimo tabulato le paghe li hanno già contati nel goduto, quindi toglierli di nuovo li conterebbe due volte. Il gestionale lo scrive in chiaro fra gli avvisi, con la data a cui è fermo il tabulato."
      },
      {
  ...
  },
  {
    ...
    "description": "La pagina Chiusura cassa è lo specchietto incassi del negozio, in cinque blocchi. 1) Incassi del giorno: totale corrispettivi dello scontrino di chiusura (con l'unica foto obbligatoria), più le vendite con fattura, uguale il totale incassato, che deve coincidere con la somma dei mezzi di pagamento (contanti, POS, pay by link, bonifico). 2) Spese pagate con i contanti e rimborsi a cliente. 3) Versamento in banca. 4) Fondo cassa contato stasera. 5) Contanti ancora da versare, contati stasera. I punti 4 e 5 sono obbligatori: senza quei due numeri la conferma si ferma e un avviso al centro dice cosa contare. Dalla foto unica il gestionale legge totale, contanti e le chiusure dei POS e li propone nei campi ancora vuoti, da controllare sempre; le spese e il versamento hanno la loro foto e la loro lettura. La pagina controlla in tempo reale che la giornata quadri, sia sugli incassi sia sul contante, e alla conferma il giorno diventa definitivo. Chi amministra (super advisor e contabile) usa la stessa pagina scegliendo il punto vendita.",
    "sections": [
      {
  ...
  },
  {
    ...
    "sections": [
      ...
      },
      {
        ...
        "body": "La scheda \"Panoramica\" mostra alcuni numeri di sintesi (percentuale di record validi, duplicati trovati, errori di mapping da risolvere, quante fonti sono attive) e due grafici: i record importati mese per mese e la distribuzione degli import per tipo di fonte. La scheda \"Cronologia\" elenca invece, in un'unica tabella, tutti i file caricati da qualunque fonte, con data, nome file, tipo di fonte, dimensione e stato: utile per ritrovare rapidamente un caricamento fatto in passato. Ricaricando lo stesso documento per lo stesso periodo, il file nuovo sostituisce il precedente: in Cronologia resta in elenco la versione che vale, non due file uguali fra cui indovinare. Il file sostituito non viene cancellato, porta l'etichetta \"sostituito\" e ricompare spuntando \"Mostra anche le versioni sostituite\". Ogni file caricato dall'Hub entra in archivio già classificato: il sistema scrive da sé l'area del gestionale a cui appartiene (Banche per gli estratti conto, Personale per i cedolini e così via), la funzione che lo ha caricato, chi lo ha caricato e il periodo, che viene letto dal nome del file quando c'è scritto, per esempio \"Dal 062026\", \"07-2026\" o \"luglio 2026\". Se nel nome non c'è nessuna data il periodo resta vuoto, perché un mese sbagliato è peggio di un mese mancante. L'unica eccezione sono i Documenti Generali, dove ci passa di tutto e l'area non si può indovinare: lì c'è una tendina \"Area del gestionale\" da scegliere al momento del caricamento, così il documento si ritrova insieme agli altri della stessa area invece di restare nel mucchio."
      }
    ],
    "faq": [
      {
  ...
  },
  {
    ...
    "faq": [
      ...
      },
      {
        ...
        "a": "Apri il pannello Gestione del fornitore (icona dei cursori), scorri fino a \"Fatture del fornitore\" e clicca il pulsante \"PDF\" sulla riga della fattura che ti interessa. Se il fornitore non ha allegato nessun PDF, il sistema te lo segnala con un messaggio: non è un errore, puoi comunque vedere la fattura con \"Apri\".\n\nDal 21 settembre 2026 il PDF si apre anche sulle fatture arrivate dal Cassetto Fiscale, cioè quelle di fine luglio in poi. Quelle fatture sono salvate in un formato diverso (dati strutturati invece del documento XML) e il pulsante \"PDF\" non riusciva a leggerne gli allegati: rispondeva \"Nessun PDF allegato\" anche quando il fornitore lo aveva messo. Erano 106 fatture su 350. Ora il documento si apre come per tutte le altre, senza dover riscaricare niente."
      },
      {
  ...
  },
  {
    ...
    "sections": [
      ...
      },
      {
        ...
        "body": "Il partitario è il registro contabile del fornitore: in colonna AVERE trovi le fatture ricevute (che aumentano il debito verso il fornitore), in colonna DARE i pagamenti effettuati e le note di credito (che lo riducono). Se una fattura è divisa in più rate e ne hai pagata solo una parte, il DARE registra SOLO l'importo effettivamente pagato (non l'intero totale della fattura) e la riga è etichettata \"Acconto\": così il saldo mostra correttamente il residuo ancora da pagare. Vale anche per gli acconti versati su una fattura unica: appena i soldi escono dal conto la riga entra in partitario, senza aspettare il saldo finale. La riga di pagamento riporta la banca e la data: se alla fattura è agganciato un movimento bancario, sono quelle del movimento (\"Pagamento — Banca — movimento bancario del GG/MM/AAAA\"), e il movimento vince su tutto il resto: una fattura chiusa a mano prima che arrivasse il bonifico non mostra più la dicitura \"Chiusura manuale\" ma il pagamento reale, con l'importo uscito dalla banca. Solo senza movimento agganciato vedi la chiusura manuale o la banca prevista sulla scadenza. Ogni riga mostra anche il saldo progressivo: se scende sotto zero (evidenziato in rosso) significa che l'azienda è ancora in debito. Puoi scegliere se ordinare i movimenti per data di emissione fattura o per data effettiva di pagamento, con il menu \"Ordina per\" in alto a destra della tabella. In fondo trovi i totali dei movimenti selezionati e il saldo corrente della scheda.",
        "steps": [
  ...
  },
  {
    ...
    "sections": [
      ...
      },
      {
        ...
        "steps": [
          ...
          "Scrivi nel tax free (Global Blue) l'IVA delle note di variazione del mese, dal dettaglio della liquidazione dello studio",
    ...
    "faq": [
      ...
      {
        "q": "Cos'è la riga \"tax free\" sotto l'IVA acquisti?",
        "a": "È l'IVA che l'azienda recupera sulle vendite tax free: quando un turista extra UE ottiene il rimborso, Global Blue emette una nota di variazione e quell'IVA torna a credito. Lo studio la registra in un registro a parte dall'estratto conto Global Blue, che non passa dallo SDI: per questo il gestionale non può calcolarla da solo. Alla conferma del mese scrivi il valore vero (per agosto 2026 erano tre note per 588,78 €); per i mesi non confermati la pagina usa la media dei mesi confermati e la segna con ≈."
      },
      {
    ...
    "sections": [
      ...
      },
      {
        ...
        "steps": [
          ...
          "In \"Richieste di riapertura dai negozi\" scrivi gli indirizzi che devono ricevere la mail quando un punto vendita chiede di riaprire una chiusura confermata: arriva con negozio, giorno, motivo e il link alla giornata. Vuoto significa che la mail va ai destinatari del report",
  ...
  },
  {
    ...
    "sections": [
      ...
      },
      {
        ...
        "body": "In alto ci sono cinque riquadri, uno per ogni canale (\"feed\") di sincronizzazione: Banche, Fatture passive, Fatture attive, Corrispettivi e Cassetto Fiscale. Ogni riquadro mostra un pallino colorato e un'etichetta di stato:\n\n• \"Aggiornato\" (pallino scuro) — l'ultimo aggiornamento è recente e regolare.\n• \"In ritardo\" (pallino arancione) — l'ultimo aggiornamento riuscito è un po' più vecchio del previsto, oppure l'ultima esecuzione è stata completata con avvisi.\n• \"Fermo\" (pallino rosso) — l'ultima esecuzione è fallita, oppure non si aggiorna da troppo tempo.\n• \"Nessuna sincronizzazione\" (pallino grigio) — quel canale non ha ancora mai girato.\n\nBanche, Fatture passive, Fatture attive e Corrispettivi si aggiornano automaticamente ogni 6 ore (alle 00, 06, 12 e 18, con le fatture mezz'ora dopo le banche); il Cassetto Fiscale una volta al giorno, alle 7:15. Il canale Corrispettivi risulta \"non ancora attivo\" finché non viene collegato. Toccando una card (o passandoci sopra il mouse) compare il dettaglio (data/ora dell'ultimo aggiornamento e, se presente, la descrizione dell'errore).\n\nFatture passive e Cassetto Fiscale portano le stesse fatture dei fornitori, per due strade diverse: le Fatture passive le prendono dal canale SDI (quelle indirizzate al nostro codice destinatario, complete di documento XML e allegati), il Cassetto Fiscale le recupera dall'archivio dell'Agenzia delle Entrate, comprese quelle che allo SDI non sono arrivate. Ogni fattura viene conteggiata una sola volta, dal canale che la scarica per primo: se in un riquadro leggi 0 documenti non vuol dire che manchi qualcosa, ma che quelle fatture erano già entrate dall'altra parte. Fino al 21 settembre 2026 il Cassetto girava alle 5:15, cioè prima del canale SDI: arrivava sempre per primo, e le Fatture passive segnavano 0 da due mesi. Ora l'ordine è invertito."
      },
      {
