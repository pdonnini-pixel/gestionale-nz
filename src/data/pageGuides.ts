  ...
  },
  {
    ...
    "sections": [
      ...
      },
      {
        ...
        "body": "Qui abbini i movimenti in uscita dal conto alle fatture fornitori corrispondenti, così il sistema sa quali fatture sono state effettivamente pagate. La scheda si divide in due viste: \"Da riconciliare\" e \"Riconciliati\". Nella vista \"Da riconciliare\", se il sistema trova un possibile abbinamento (per importo, nome fornitore e data) lo propone nella sezione \"Abbinamenti suggeriti\" con una percentuale di affidabilità: puoi confermarlo o rifiutarlo, anche in blocco per più righe insieme. Se selezioni un movimento dall'elenco a sinistra, a destra vedi le fatture proposte come possibile corrispondenza (o puoi cercarne una manualmente). Nella vista \"Riconciliati\" trovi tutti i movimenti già abbinati, con fornitore e numero fattura: da qui puoi annullare un abbinamento sbagliato con \"Annulla abbinamento\" (la fattura torna aperta, il movimento torna da riconciliare). Vengono proposti come abbinamento suggerito solo i casi affidabili: l'importo del movimento deve coincidere con il residuo della fattura (entro il 5%) e l'affidabilità deve essere almeno del 70%; i match con importo lontano non compaiono. Ogni movimento viene verificato non solo contro le fatture ancora aperte, ma SEMPRE anche contro le fatture già segnate come pagate a mano ma senza movimento bancario collegato: se la contabile aveva chiuso una fattura manualmente, il bonifico che l'ha pagata resta \"orfano\" e il sistema propone comunque l'aggancio (la fattura resta pagata, si collega solo il movimento). Questi suggerimenti sono contrassegnati con l'etichetta \"chiusa a mano\". Sotto i suggerimenti ad alta affidabilità c'è una seconda sezione \"Da verificare — beneficiario dalla causale\": per i bonifici il sistema legge il beneficiario scritto nella causale (\"…a favore di: NOME…\") e lo abbina alla fattura di quel fornitore — aperta oppure già chiusa a mano — quando l'importo coincide (entro il 5%). Serve sia il nome sia l'importo: così non propone abbinamenti sbagliati per pura coincidenza di importo (es. bonifici a fondi o assicurazioni come Azimut o Mediolanum, che non sono fornitori con fattura). Ogni riga mostra il beneficiario e la fattura proposta (con l'etichetta \"chiusa a mano\" quando la fattura era già stata pagata manualmente); l'abbinamento va confermato una alla volta (non c'è \"conferma tutti\"), e \"Nascondi\" toglie una proposta che non ti interessa. I movimenti che non sono pagamenti a fornitori (F24, bolli, CBILL/PagoPA, giroconti, commissioni, ricariche) non vengono proposti. Quando invece il numero della fattura è scritto per esteso nella causale del movimento e l'importo coincide, l'abbinamento si chiude già da solo (non serve confermarlo); le fatture già chiuse a mano non si chiudono mai in automatico, richiedono sempre la tua conferma. Infine, quando un unico bonifico salda più fatture dello stesso fornitore (es. 466,95 = 155,65 + 311,30), compare la sezione \"Pagamenti raggruppati — un bonifico, più fatture\": il sistema cerca la combinazione di 2-3 fatture di quel fornitore la cui somma coincide con l'importo del movimento e ti propone il gruppo; confermandolo, tutte le fatture del gruppo vengono agganciate a quel singolo movimento in un colpo solo (operazione tutto-o-niente: se la somma non torna esatta non abbina nulla).",
        "steps": [
