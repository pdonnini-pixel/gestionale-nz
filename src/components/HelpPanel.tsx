...
import {
  ...
  LayoutDashboard, Store, Receipt, Landmark, Users,
...
// Costruisce il testo di contesto da passare all'edge function help-chat:
// riusa i testi guida già scritti per la pagina (descrizione + tips + faq),
// così l'AI risponde allineata alla guida senza duplicare nulla nel backend.
function buildPageContext(content: HelpContent | null): string {
  if (!content) return ''
  ...
  if (content.faq.length > 0) {
    parts.push('Domande frequenti:\n' + content.faq.map((f) => `D: ${f.q}\nR: ${f.a}`).join('\n'))
  }
  return parts.join('\n\n')
}

// Titolo leggibile per pagine senza guida statica (la chat è comunque
// disponibile ovunque, come richiesto). Es. "/margini-outlet" -> "Margini Outlet".
...
export default function HelpPanel() {
  ...
  const pageTitle = content?.title || titleFromPath(path)
  const pageContext = buildPageContext(content)

  // Su pagine senza guida statica, apri direttamente la chat.
  const effectiveTab: 'guida' | 'chat' = content ? tab : 'chat'

  // Reset stato all'uscita dalla pagina
  useEffect(() => {
    setExpandedFaq(null)
    setTab('guida')
  }, [path])
    ...
    <>
      ...
      {open && (
        <div className="fixed bottom-20 right-6 z-40 w-[380px] max-w-[calc(100vw-3rem)] h-[70vh] max-h-[560px] bg-white rounded-2xl border border-slate-200 shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-2">
          ...
          {/* Body */}
          {effectiveTab === 'chat' ? (
            <AssistantChat path={path} pageTitle={pageTitle} pageContext={pageContext} />
          ) : (
