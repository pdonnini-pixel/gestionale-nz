...
export default function GlobalSearch({ open: openProp, onClose }: GlobalSearchProps) {
  ...
  async function doSearch(q: string) {
    ...
    try {
      ...
      if (movements.data?.length) res.movements = movements.data.map(m => ({ id: m.id as string, title: m.counterpart || m.description?.slice(0, 50) || '—', subtitle: `€${Number(m.amount || 0).toLocaleString('de-DE')} — ${m.date}`, url: '/banche', fullTitle: m.counterpart || m.description || '' }))
