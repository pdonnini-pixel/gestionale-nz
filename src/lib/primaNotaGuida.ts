...
export function buildGuidaRows(g: GuidaInput): StyledRow[] {
  const rows: StyledRow[] = [
    ...
    t('Codici negozi', 'Una riga per negozio con i codici con cui compare sui documenti: punto vendita e contratto Nexi, codici esercente Amex, numero del terminale POS per banca, parola chiave dei versamenti. Gli estratti Nexi e Amex non riportano il nome del negozio: si cerca il codice qui.'),
    ...
    t('Righe in corsivo grigio', 'Dettaglio sotto un movimento: una riga per fattura («↳ di cui fattura») quando il movimento ne salda più d\'una, poi l\'eventuale resto e il totale; sotto una ricarica della prepagata, il rimando al foglio della carta («↳ ricarica prepagata»).'),
    ...
    t('Tipo movimento', 'Che cos\'è la riga: Pagamento fornitore (con «(RiBa)» se le fatture sono a ricevuta bancaria), F24 / imposte, Stipendi, Incasso POS, Versamento contanti, Carta di credito (l\'addebito mensile dell\'estratto), Carta di debito (POS), Ricarica carta prepagata (soldi passati dal conto alla Tasca), Finanziamento, Spese e commissioni bancarie, Giroconto / prelievo, Incasso cliente, Rimborso, Da chiarire.'),
    ...
    t('Ricarica della prepagata', 'L\'uscita «Ricarica carta prepagata» NON è una spesa: sono soldi passati dal conto alla carta Tasca. Sotto la riga c\'è «↳ ricarica prepagata», che dice in quale foglio della carta quei soldi sono stati spesi, quante spese ci sono nel mese e il saldo della carta a fine mese. Se la riga è ambra, l\'estratto della carta di quel mese non è stato importato nel gestionale.'),
    ...
    t('Come si cerca', 'Filtro sulla colonna Outlet per vedere gli incassi di un solo negozio; filtro su Tipo per separare POS, Amex, versamenti e bonifici. Un outlet vuoto significa che l\'accredito non è stato attribuito.'),
    t('Bonifici di clienti', 'Un cliente che paga con bonifico è un incasso del negozio come il POS: il negozio lo scrive nella chiusura di cassa sulla riga «Bonifico», e il gestionale abbina il bonifico arrivato in banca (o due bonifici che sommano) a quella riga. L\'outlet e la data di riferimento vengono da lì. Un bonifico di cliente senza outlet non è stato registrato in nessuna chiusura.'),
    b(),
    s('Codici negozi'),
    t('Cosa trovi', 'Per ogni negozio: «Nexi punto vendita» (il codice LN… scritto a pagina 2 dell\'estratto Nexi, riga «VICOLO - punto vendita cod.»), «Nexi contratto» (il Payment Contract PC…) e il regime di accredito (al netto delle commissioni, o al lordo con le commissioni addebitate a parte); «Amex codici esercente» (le 10 cifre accanto al nome nell\'estratto Amex, e in coda alla causale dell\'SDD delle commissioni); i numeri di terminale POS che compaiono nelle causali degli accrediti BCC e MPS; la parola chiave dei versamenti di contante.'),
    t('Come si cerca', 'Ctrl+F sul codice che vedi sul documento: la prima colonna dice il negozio. I codici sono fissi, non cambiano da un mese all\'altro.'),
    b(),
    ...
    t('Riscontro banca', 'Per le carte di credito la riga dice «estratto addebitato il …» quando l\'addebito unico dell\'estratto è sul conto, oppure «in attesa dell\'addebito» finché la banca non lo fa (di solito il 25 del mese dopo: l\'estratto di agosto si addebita a fine settembre). Per la prepagata: le ricariche dicono «addebito in banca il …» (o «ricarica non trovata in banca» se manca), le spese dicono «spesa sulla carta: non passa dal conto» perché escono dal saldo della carta, non dal conto.'),
    ...
    t('Prepagata: come si riconcilia', 'Tre passi. 1) Nel foglio del conto, ogni uscita «Ricarica carta prepagata» è un giroconto verso la carta, non un costo: la stessa cifra compare nel foglio della carta come riga positiva, con «Riscontro banca: addebito in banca il …». 2) I costi sono le righe negative del foglio della carta: ognuna con esercente, importo, commissione e, quando c\'è, la fattura dello Scadenzario che salda. 3) Il saldo finale del foglio è quanto resta sulla carta a fine mese e deve essere il saldo del conto «carta prepagata» in contabilità: saldo iniziale + ricariche − spese − commissioni. Se torna, le ricariche del conto sono spiegate per intero dalle spese della carta e da ciò che resta sopra.'),
