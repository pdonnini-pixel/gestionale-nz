...
describe('buildCashClosingsSheets', () => {
  it('riepilogo giorni × outlet e un foglio per outlet con i canali', () => {
    ...
    expect(v[0].slice(0, 4)).toEqual(['Data', 'Totale corrispettivi', 'Contanti', 'POS MPS'])
    expect(v[1].slice(1, 4)).toEqual([1019.63, 95.1, 924.53])
    expect(v[1][6]).toBe(500)
    expect(v[1][9]).toBe('Confermata')
    expect(v[1][10]).toBe('Anna')
    expect(v[2][1]).toBeNull()
    expect(v[3][0]).toBe('Totale')
    expect(v[3][1]).toBe(1019.63)
    const t = sheets[2].aoa
    expect(t[2][1]).toBe('chiuso')
    expect(t[2][8]).toBe('Negozio chiuso')
  })
})
