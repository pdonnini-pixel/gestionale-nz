// Edge Function: acube-ob-connect-request
// Lancia una richiesta di consenso PSD2 per un Business Registry e ritorna l'URL
// del mini-sito A-Cube dove l'utente sceglie la banca + autorizza i conti.
// Endpoint A-Cube: POST /business-registry/{fiscalId}/connect
//
// Body:
//   {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const locale: string = (body.locale ?? "it").toString();
    ...
    // Trova il BR locale per linkare il consenso
    const { data: br } = await supabase.from("acube_business_registries").select("uuid").eq("fiscal_id", fiscalId).eq("stage", stage).maybeSingle();
    if (!br) return jsonError(404, `Business Registry sandbox non trovato per fiscalId=${fiscalId}. Chiama prima acube-ob-br-upsert.`);

    let jwt = await getCachedJwt(supabase, stage);

    // POST /business-registry/{fiscalId}/connect
    const doPost = async (j: string) => fetch(`${baseUrl}/business-registry/${encodeURIComponent(fiscalId)}/connect`, {
      method: "POST",
    ...
    if (!resp.ok) return jsonError(resp.status, `A-Cube connect HTTP ${resp.status}: ${(await resp.text()).slice(0, 300)}`);
    const data = await resp.json();
    const url: string | undefined = data.url ?? data.authorisationUrl ?? data.connect_url;
    if (!url) return jsonError(502, `A-Cube connect missing url field. Body: ${JSON.stringify(data).slice(0, 300)}`);

    const { data: cons, error: consErr } = await supabase.from("acube_consents").insert({
      business_registry_uuid: br.uuid,
      status: "pending",
      connect_url: url,
      days: data.days ?? null,
      notice_level: data.noticeLevel ?? data.notice_level ?? null,
    }).select("id").single();
    if (consErr) return jsonError(500, `Insert acube_consents failed: ${consErr.message}`);

    return jsonOk({ url, consent_id: (cons as { id: string }).id });
  } catch (e) {
