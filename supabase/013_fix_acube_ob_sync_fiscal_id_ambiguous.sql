-- 013_fix_acube_ob_sync_fiscal_id_ambiguous.sql
-- FIX 1: cron "acube-ob-sync-every-6h" fallisce dal 2026-05-29 con
--   ERROR: column reference "fiscal_id" is ambiguous
-- Causa: la funzione dichiara il parametro OUT "fiscal_id" (RETURNS TABLE)
--   e nel ciclo FOR fa SELECT ... fiscal_id ... FROM acube_business_registries
--   senza qualificare la colonna -> conflitto variabile/colonna.
-- Soluzione: alias di tabella "abr" e colonne qualificate nel solo ciclo FOR.
--
-- FIX 2 (2026-06-01): una volta rimosso l'errore "ambiguous" emerge un 2o blocco:
--   ERROR: there is no unique or exclusion constraint matching the ON CONFLICT specification
-- Causa: gli unici indici unique su bank_transactions.acube_dedup_hash sono PARZIALI
--   (WHERE acube_dedup_hash IS NOT NULL); un "ON CONFLICT (acube_dedup_hash)" senza
--   predicato non puo' inferire un indice parziale come arbitro.
-- Soluzione: qualificare l'ON CONFLICT con "WHERE acube_dedup_hash IS NOT NULL".
--   Il dedup resta identico (canonical_hash MD5 e' sempre non-null). Non distruttivo.
--
-- Entrambe le modifiche sono non distruttive (CREATE OR REPLACE) e reversibili.
-- DA APPLICARE SU TUTTI E 3 I TENANT: NZ, Made, Zago (regola di parità).
