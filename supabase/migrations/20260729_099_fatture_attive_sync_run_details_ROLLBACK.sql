-- ROLLBACK 099 — rimuove il dettaglio delle fatture attive
--
-- 1. Elimina le righe di dettaglio del feed fatture_attive (incluse quelle di
--    backfill): sono righe di sola osservabilità, nessun dato operativo.
-- 2. Ripristina la funzione outbound alla versione 091 (senza logging dettaglio).

DELETE FROM public.sync_run_details WHERE feed = 'fatture_attive';

-- Ripristino funzione outbound versione 091 (nessuna insert in sync_run_details).
