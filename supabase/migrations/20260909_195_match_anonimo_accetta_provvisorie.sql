-- 20260909_195 — Il match su causale anonima considera anche le scadenze chiuse in via provvisoria.
--
-- Con la migration 194 le scadenze pagate in contanti o con carta si chiudono da sole alla
-- scadenza, in via provvisoria e senza prova bancaria. Il movimento però può arrivare dopo:
-- da A-Cube oppure da un ESTRATTO CONTO caricato a mano (è l'altra sorgente di movimenti, e
-- per le carte spesso è l'unica). Quando arriva, la scadenza va agganciata e la chiusura
-- diventa definitiva (il trigger update_payable_status azzera is_provisional_paid da solo).
--
-- Il ramo dei flussi CBI anonimi accettava le scadenze già 'pagato' solo se chiuse A MANO
-- (migr. 192), quindi le provvisorie restavano invisibili al motore e il loro movimento
-- sarebbe rimasto aperto per sempre. Qui si aggiunge il ramo simmetrico, con la stessa
-- prudenza: il movimento deve cadere in una finestra ragionevole intorno alla data di
-- pagamento (-30 / +60 giorni, più larga in avanti perché l'addebito della carta arriva
-- anche un mese e mezzo dopo la spesa).
--
-- Unica modifica: la CTE `cand` dentro try_match_amount_bank_transaction. Resto invariato.
-- Rollback in 20260909_195_match_anonimo_accetta_provvisorie_ROLLBACK.sql
...
BEGIN
  ...
  WITH cand AS (
    ...
    WHERE p.company_id = v_bt.company_id
      ...
      AND ( p.status IN ('da_pagare', 'in_scadenza', 'scaduto')
            ...
            OR ( p.status = 'pagato' AND COALESCE(p.is_provisional_paid, false)
                 AND ( p.payment_date IS NULL
                       OR v_bt.transaction_date
                            BETWEEN p.payment_date - INTERVAL '30 days'
                                AND p.payment_date + INTERVAL '60 days' ) ) )
      AND NOT EXISTS (
  ...
  IF v_only.status = 'pagato' AND (COALESCE(v_only.closed_manually, false) OR COALESCE(v_only.is_provisional_paid, false)) THEN
    ...
    VALUES (v_bt.company_id, p_bt_id, v_only.id, 'auto_exact', 95, 'applied', v_only.gross_amount,
            'auto: IMPORTO BONIFICI netto esatto e univoco (flusso CBI anonimo) — chiusa a mano o provvisoria, solo aggancio');
  ELSE
