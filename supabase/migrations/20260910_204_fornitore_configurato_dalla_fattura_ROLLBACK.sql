-- ROLLBACK 20260910_204 — il fornitore torna a nascere vuoto
--
-- Riporta il bridge A-Cube alla forma della 176 (fornitore creato con 30 gg e
-- bonifico d'ufficio, nessuna lettura del profilo) e rimuove le funzioni nuove.
--
-- I DATI GIA' COMPILATI NON VENGONO TOCCATI: sono campi che prima erano vuoti e
-- ora contengono cio' che dice la fattura. Per riportare anche quelli allo stato
-- del 10/09/2026 c'e' la copia integrale in public.suppliers_backup_profilo_20260910
-- (da usare con un UPDATE mirato, dopo aver verificato riga per riga: nel frattempo
-- l'anagrafica puo' essere stata corretta a mano).

BEGIN;

DROP FUNCTION IF EXISTS public.rpc_backfill_supplier_profiles(integer);
DROP FUNCTION IF EXISTS public.fn_supplier_profile_from_invoice(uuid, uuid, text, date, uuid);
DROP FUNCTION IF EXISTS public.fn_invoice_cedente_profile(text);

ALTER TABLE public.suppliers
  DROP COLUMN IF EXISTS profile_from_invoice_id,
  DROP COLUMN IF EXISTS profile_from_invoice_at,
  DROP COLUMN IF EXISTS profile_from_invoice_fields;
