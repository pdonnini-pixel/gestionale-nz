-- 20260731_131_acube_bridge_skip_reverse_charge_autofattura.sql
--
-- FIX CICLO PASSIVO — i documenti di INTEGRAZIONE / AUTOFATTURA reverse charge
-- (TD16 interno, TD17 estero, TD18 intra-UE, TD19 art.17 c.2) NON sono debiti
-- verso il fornitore: sono documenti IVA auto-emessi dal cessionario (New Zago)
-- e numerati col suo sezionale interno (es. "34/A"). Il debito reale resta sulla
-- fattura originale del fornitore (TD01/TD24...).
--
-- Prima di questa migration il bridge `sync_acube_sdi_passive_to_payable` creava
-- una scadenza (payable) anche per questi documenti, generando "fatture fantasma"
-- che sporcavano lo scadenzario e gonfiavano il totale pagato (caso segnalato da
-- Sabrina — New Zago, luglio 2026).
--
-- FIX: si continua ad archiviare la fattura elettronica (per l'IVA / cassetto
-- fiscale), ma si esce PRIMA di creare qualsiasi payable quando il document_type
-- e' un'integrazione/autofattura reverse charge. Nessun altro ramo e' toccato.
--
-- REGOLA #0 (parita' tenant): applicare su NZ + Made + Zago.
-- La bonifica dei dati storici (soft-hide dei payables gia' creati) e' separata e
-- riguarda SOLO i tenant che li hanno (attualmente solo NZ) — vedi file NZ_ONLY.
...
begin
  ...
  -- Documenti di integrazione / autofattura reverse charge: TD16 (interno),
  -- TD17 (estero), TD18 (intra-UE), TD19 (art.17 c.2). Sono documenti IVA
  -- auto-emessi dal cessionario, NON un debito verso il fornitore. Si archivia
  -- la fattura elettronica (sopra) ma NON si genera alcuna scadenza/payable.
