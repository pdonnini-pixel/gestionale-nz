-- =====================================================================
-- 197 — Riscontro banca: accredito parziale resta «in attesa»
-- ---------------------------------------------------------------------
-- Gli accrediti MPS di una giornata arrivano in due righe (Bancomat e Nexi)
-- che possono avere valuta diversa: con una sola riga arrivata il riscontro
-- segnava subito «differenza». Ora, se l'accreditato e' inferiore al
-- dichiarato e la giornata ha meno di 5 giorni, la riga resta «in attesa»
-- (con l'importo parziale visibile); dopo 5 giorni diventa «differenza».
-- Solo funzione, stessa firma. Tenant: NZ, Made, Zago.
-- =====================================================================
...
BEGIN
  ...
  WITH grp AS (
    SELECT l.closing_id, public.cash_bank_norm_code(ch.terminal_code) AS code, ch.kind,
           ...
           max(ch.bank_tolerance_pct) AS tol_pct, min(c.closing_date) AS closing_date
    ...
    SELECT g.line_ids, g.declared, b.tot, b.last_at,
           CASE WHEN abs(b.tot - g.declared) <= GREATEST(p_tolerance, round(g.declared * COALESCE(g.tol_pct, 0) / 100, 2))
                THEN 'accreditato'
                -- accredito parziale (es. Bancomat arrivato, Nexi non ancora): resta in attesa per 5 giorni
