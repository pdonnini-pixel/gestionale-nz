-- =====================================================================
-- 224 — Riscontro cassa/banca: l'Amex torna un canale suo e la tolleranza
--       smette di essere un numero a pioggia
-- ---------------------------------------------------------------------
-- Correggge due premesse sbagliate (COMMISSIONI_INCASSO_NOTES.md):
--
-- 1) La 195 dice che gli accrediti MPS arrivano al netto «tranne un
--    terminale». Quel terminale e' Valdichiana, ed e' l'unico al lordo non
--    per caso ma per CONTRATTO (Payment Contract PC0001000583: al lordo).
--    Gli altri sei punti vendita hanno contratti al netto. La tolleranza
--    dell'1,5 % messa su tutti i canali POS copre quindi due situazioni
--    opposte: dove l'accredito e' lordo non dovrebbe esserci scarto alcuno.
--
-- 2) La NZ_ONLY 198 dice che l'Amex passato dal POS MPS arriva DENTRO
--    l'accredito del giorno. E' falso: l'Amex e' sempre una riga separata,
--    al lordo, il giorno dopo la vendita. Verificato su 271 righe su 271
--    negli estratti conto Amex di gennaio-agosto 2026. Degradando il canale
--    «POS MPS Amex» a tipo pos con lo stesso codice terminale del POS, le
--    due righe della chiusura finiscono nello stesso gruppo e il riscontro
--    scrive su ENTRAMBE il totale del terminale: a settembre la riga Amex
--    di Palmanova risultava «accreditato 25.648,94» a fronte di 5.300,20
--    dichiarati. Un numero falso sotto gli occhi della cassiera.
--
-- Cosa cambia:
--   * nuova colonna outlet_payment_channels.settlement_mode ('lordo'/'netto'):
--     dove l'accredito e' lordo la tolleranza torna al centesimo, dove e'
--     netto lo scarto ammesso resta ed e' la commissione trattenuta.
--   * il riscontro riconosce l'accredito Amex dentro il flusso POS: fra i
--     movimenti del terminale cerca quello che vale esattamente quanto la
--     chiusura dichiara sul canale Amex di quel giorno, lo abbina al canale
--     Amex e lascia gli altri al canale POS.
--
-- Additiva e idempotente. Tenant: NZ, Made, Zago (colonna e funzione).
-- I canali di NZ li rimette a posto la NZ_ONLY 225.
-- =====================================================================

-- ---------------------------------------------------------------------
-- 1. Regime di accredito sul canale
-- ---------------------------------------------------------------------
ALTER TABLE public.outlet_payment_channels
  ADD COLUMN IF NOT EXISTS settlement_mode text NOT NULL DEFAULT 'netto';
ALTER TABLE public.outlet_payment_channels
  DROP CONSTRAINT IF EXISTS outlet_payment_channels_settlement_mode_check;
ALTER TABLE public.outlet_payment_channels
  ADD CONSTRAINT outlet_payment_channels_settlement_mode_check
  CHECK (settlement_mode IN ('lordo', 'netto'));
COMMENT ON COLUMN public.outlet_payment_channels.settlement_mode IS
  'lordo = in banca arriva il transato pieno e la commissione e'' addebitata a parte (confronto al centesimo); netto = commissione trattenuta alla fonte, lo scarto ammesso e'' bank_tolerance_pct.';

-- Gli Amex accreditano sempre al lordo: e' vero su tutti i tenant.
UPDATE public.outlet_payment_channels
   SET settlement_mode = 'lordo'
 WHERE kind = 'pos_amex' AND settlement_mode <> 'lordo';
...
-- Verifica:
--   SELECT label, kind, settlement_mode, bank_tolerance_pct FROM outlet_payment_channels ORDER BY label;
--   SELECT match_cash_closings_with_bank(NULL, 30, 0.01);
