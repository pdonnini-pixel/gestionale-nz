-- ROLLBACK 20260903_170_bridge_termini_fattura_vincono
-- Ripristina il bridge alla versione 131 (piano fornitore PRIMA dei termini in
-- fattura a rata unica) e rimuove le funzioni di supporto. Nessun dato toccato.
-- La definizione qui sotto e' copiata tale e quale da
...
DROP FUNCTION IF EXISTS public.fn_parse_invoice_condizioni_json(jsonb);
DROP FUNCTION IF EXISTS public.fn_parse_invoice_condizioni(text);
DROP FUNCTION IF EXISTS public.fn_sdi_mp_label(text);
DROP FUNCTION IF EXISTS public.fn_sdi_mp_to_payment_method(text, text);
