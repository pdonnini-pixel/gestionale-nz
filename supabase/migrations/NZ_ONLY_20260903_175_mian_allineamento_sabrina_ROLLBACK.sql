-- ROLLBACK di NZ_ONLY_20260903_175_mian_allineamento_sabrina.sql
-- Toglie le 8 rate aggiunte (righe create dalla migration, mai esistite prima) e ripristina le 7 righe dal backup.
BEGIN;

DELETE FROM public.payable_actions
WHERE note LIKE 'Allineamento MIAN agli elenchi scadenze di Sabrina (03/09/2026)%';

DELETE FROM public.payables
WHERE supplier_name ILIKE 'MIAN%' AND invoice_number IN ('839','847','865','883') AND installment_number IN (2, 3)
  AND notes = 'Rata generata da ricalcolo piano RI.BA (03/09/2026, elenco scadenze MIAN di Sabrina)'
  AND id NOT IN (SELECT id FROM public._bkp_mian_allineamento_sabrina_20260903);
