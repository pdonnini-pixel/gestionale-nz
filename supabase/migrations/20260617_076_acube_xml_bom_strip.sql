-- 076 — strip BOM dall'XML A-Cube
--
-- Alcune fatture A-Cube servono il FatturaPA XML con un BOM UTF-8 (U+FEFF)
-- prima di "<?xml". La 075 controllava left(ltrim(content),1)='<' (ltrim NON
-- rimuove il BOM) → quelle fatture restavano senza XML reale. Qui togliamo il
-- BOM (e gli spazi iniziali) prima di salvare, sia nella funzione di pull sia
-- nel trigger (difesa anche per il path webhook/manuale).

-- ─── trigger: BOM-tolerante sull'xml ─────────────────────────────────────
...
declare
  ...
  v_net numeric; v_vat numeric; v_xml text; v_clean text;
begin
  ...
  v_clean := ltrim(NEW.xml_content, chr(65279) || E' \t\r\n');
  v_xml := case when NEW.xml_content is not null and left(v_clean,1) = '<' then v_clean else NEW.payload::text end;
...
-- ─── funzione di pull: strip BOM dall'XML scaricato ──────────────────────
